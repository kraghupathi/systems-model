...
*** SSHD config file
This file sets up the sshd configurations. The commented parameters
are the default values set for the admin awareness. 
This configuration file sets the following:
    + Permit root login as key based not as password based
    + Disables the password based authentication
    + Enables public key authentication
    + Does not permit empty passwords
    
#+BEGIN_SRC SHELL :tangle roles/common/files/sshd_config
...
*Note:* Alternative work around for config file needs to be looked
over, keeping the functionality with representation transformation.
...
#   This below implementation using lineinfile has a problem of appending the commands at every run.
#   Needs Troubleshooting.
#  
# - name: Permit root login without-pasword(key based) and backup the file before modifying
#   lineinfile: dest=/etc/ssh/sshd_config regexp='^#PermitRootLogin *' line='PermitRootLogin without-password' backup=yes state=present
