...
access. Password based access is disabled and authentication based on
public key is enabled. A good documentation on SSH can be found [[http://en.wikipedia.org/wiki/Secure_Shell][HERE]].
...
Following history parameters are set for audit purpose:
 - HISTTIMEFORMAT :: is used to display the time stamp along with the
                     command history
...
The above parameters are set and configured in the history
configuration file.
...
ssh, iptables, email and fail2ban.
