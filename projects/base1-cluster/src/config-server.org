...
** Functional Requirements
  ...
  1. <<freq1>> Configuration server will configure all the nodes in the
     cluster including itself and ensures that the configuration is consistent.

     Being =consistent= implies that the configuration currently running on the
     machine does not deviate from the release configuration.

  2. <<freq2>> Configuration server has only one `vlead-jockey' user and
     it has sudo permissions to run all commands.

  3. <<freq3>> Only authorized VLEAD members can log-in as 'vlead-jockey' user.

** Security Requirements
   1. <<sreq1>> SSH/Port 22 accessible *ONLY* from IIIT IP range.
   2. <<sreq2>> Disable root login via ssh.
   3. <<sreq3>> Limit rate of new connections on port 22.
...
#+CAPTION:  Config Server Design Diagram
...
|-----------------+-----------------------------------------------------------------|
| [[freq1]]           | At regular intervals, defined firewall rules are applied        |
|                 | thus ensuring consistency                                       |
|-----------------+-----------------------------------------------------------------|
| [[freq2]]           | A user 'vlead-jockey' is created and sudo permissions are given |
|                 | to it.                                                          |
|-----------------+-----------------------------------------------------------------|
| [[freq3]]           | public keys of VLEAD users are copied to the authorized keys    |
|                 | of the Configuration server.                                    |
|-----------------+-----------------------------------------------------------------|
| [[sreq1]]           | This is achieved by building the appropriate rules and          |
|                 | ensuring iptables service is running.                           |
|-----------------+-----------------------------------------------------------------|
| [[sreq2]]           | This is achieved by configuring the sshd_config file and        |
|                 | ensuring sshd service is running                                |
|-----------------+-----------------------------------------------------------------|
| [[sreq3]]           | This is taken care by the common role. For more information     |
|                 | refer to [[common.org]]                                             |
|-----------------+-----------------------------------------------------------------|
...
|--------------------+-------------------------------+-------------------|
| firewall           | =/etc/sysconfig/iptables/=    | ansible Role      |
|--------------------+-------------------------------+-------------------|
| sshd               | =/etc/sshd_config=            | ansible Role      |
|--------------------+-------------------------------+-------------------|
| nameserver         | =/etc/resolve.conf=           | NA                |
|--------------------+-------------------------------+-------------------|
| hostname           | =/etc/hosts=                  | Provisioning      |
|--------------------+-------------------------------+-------------------|
| IP                 | ifconfig                      | NA                |
|--------------------+-------------------------------+-------------------|
...
#+BEGIN_SRC yaml :tangle roles/config-server/templates/iptables :eval no
*filter
:INPUT ACCEPT [0:0]
:FORWARD ACCEPT [0:0]
:OUTPUT ACCEPT [0:0]
...
#+BEGIN_SRC yaml :tangle roles/config-server/templates/iptables
-A INPUT -i lo -j ACCEPT
#+END_SRC


*** Rule for related and established connections from other machines
    Allow config-server to continue already related and established
...
#+BEGIN_SRC yaml :tangle roles/config-server/tasks/main.yml :eval no
---
...
** Putting it together
   ...
   each role.  The top level script is =build/code/config-server.yml=
...
*** Domain name and IP address of server s
    #+BEGIN_EXAMPLE
    - <hostname> :: config-server.staging.virtual-labs.ac.in
    - <ip address> :: 10.4.14.101
   #+END_EXAMPLE
    
    Edit =/etc/hosts= to set the hostname of the configuration server and
    ensure that the below line is the first entry in the file.
   #+BEGIN_SRC 
   127.0.0.1 config-server.staging.virtual-labs.ac.in config-server
   #+END_SRC
...
* Test Cases
** Test case ID :: config-server-test-case-1
*** Description
...
*** Result
    #+BEGIN_EXAMPLE
    Last login: Mon Mar 16 02:28:31 2015 from config-server.staging.virtual-labs.ac.in
    [vlead-jockey@localhost ~]$
    #+END_EXAMPLE

*** Test Passed :: Yes
** Test case ID :: config-server-test-case-2
*** Description
    Checking ports
*** Steps to run the test case
