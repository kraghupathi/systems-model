...
related files, and (b) parameterizing (using variables and
other mechanisms) configuration information across servers.


* Requirements 
** Functional Requirements 
*COMMENT* Cluster should contain a configuration manager, also table gets
shifted to the design part. Also, in table mention which service is
done as part of common or ansible. Explain a little if possible 

The cluster allows its configuration server to be accessed (via ssh)
and populated with a set of ansible scripts.  These scripts are then
run on the configuration server periodically to configure the following:
...
** Security Requirements
 ...
 1) The cluster's configuration server is accessible from the outside
    only through =ssh= and that too from a fixed IP network (IIIT).

** Performance Requirements
*COMMENT* - Not in this release
