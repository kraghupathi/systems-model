...
This document describes the specification, design and implementation of a cluster
of one machine consisting of one server running the [[http://ansible.com][ansible]] software.

A cluster is a network of /servers/.  Each server runs /services/
(e.g., firewall, sshd, etc.) and the server and the services need to
be /configured/.  A configuration is either a set of rules (as in
firewall and sshd) or a mapping (in the the case of network
interfaces, hostname, etc.).

Configurations are installed either by populating configuration files
(e.g., =/etc/sysconfig/iptables= or =/etc/sshd_config=) or by running
shell commands that effect a particular configuration (e.g., =adduser
vlead-jockey= for adding a user on the server).

=Ansible= is a tool that allows installation of configurations on a
target server via ansible scripts written in the =yml= notation.
Ansible also allows arbitrary commands to be specified for execution
on a target server.  Ansible simplifies the process of configuring a
server by (a) keeping all the configuration information in related
files, and (b) parameterizing (using variables and other mechanisms)
configuration information across servers.

* Requirements 
** Functional Requirements 
   Current version - =v0.0.1= - of the cluster contains one node - configuration
   manager.  The cluster is configured and managed by the configuration
   manager.  The entry point to the cluster is configuration server.

   Access to the configuration manager is via ssh.  Configuration and management
   of the cluster is done using a set of ansible scripts.  The configuration
   server ensures there is no deviation in the current configuration of the
   cluster from the released version by periodically applying the
   configuration.

** Security Requirements

  The configuration server is accessible from a fixed IP network (IIIT)
  only through =ssh=
...
** Nodes
   The cluster contains the following machine:
 - [[./config-server.org][Configuration Server]]
