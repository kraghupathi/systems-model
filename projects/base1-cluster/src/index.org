...
#+BEGIN_COMMENT
 remove of the Cluster in this and next two
#+END_COMMENT
...
The cluster allows its ansible server to be accessed (via ssh) and
populated with a set of ansible scripts.  These scripts are then run
on the ansible server periodically to configure the following:
...
#+BEGIN_COMMENT
 ansible server to configuration server
#+END_COMMENT
