...
** Functional Requirements 
   Cluster contains a configuration manager, and allows it to be accessed
   (via ssh) and populated with a set of ansible scripts.  These scripts
   are then run on the configuration server periodically.

** Security Requirements

  The cluster's configuration server is accessible from the outside
  only through =ssh= and that too from a fixed IP network (IIIT).
...
- The table below summarizes the configurations of the machine
  
|--------------------+-------------------------------+------------------------|
| *Service/Property* | *Relevant File or Command*    | *Associated with Role* |
|--------------------+-------------------------------+------------------------|
| firewall           | =/etc/sysconfig/iptables/=    | Ansible                |
|--------------------+-------------------------------+------------------------|
| sshd               | =/etc/sshd_config=            | Ansible                |
|--------------------+-------------------------------+------------------------|
| nameserver         | =/etc/resolve.conf=           | NA                     |
|--------------------+-------------------------------+------------------------|
| hostname           | =/etc/hostname=               | Provisioning           |
|--------------------+-------------------------------+------------------------|
| IP                 | ifconfig                      | NA                     |
|--------------------+-------------------------------+------------------------|
| fail2ban           | =/etc/fail2ban/jail.local=    | Common                 |
|--------------------+-------------------------------+------------------------|
| logwatch           | =/etc/logwatch/logwatch.conf= | Common                 |
|--------------------+-------------------------------+------------------------|
