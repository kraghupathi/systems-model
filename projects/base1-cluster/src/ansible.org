...
*** Loop Back
    The ansible server accepts all connections from loop back.
...
*** Ongoing Connections
    The server allows all  ongoing connections
...
** Disable root login via ssh
  On the Ansible server, root login via =ssh= is disable
  altogether. 
...
* Testcases
** SSH should be allowed only from the IIIT network using =ansible= user.
