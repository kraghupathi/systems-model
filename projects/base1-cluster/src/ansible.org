...
** Functional Requirements
  ...
  2. Only one user called `ansible` is created on the Ansible server. This
     user has sudo permissions, to run all commands.

  3. Only VLEAD authorized users log-in as the `ansible` user.
...
configure the machine. These scripts are organized in the following way:
...
*** Domain name and IP address of Ansible machine
    - Domain name :: =ansible.sraging.virtual-labs.ac.in=
...
** SSH should be allowed only from STPI(196.12.53.130).
   Test ansible is able to ssh from STPI
   #+BEGIN_SRC shell
    ssh root@<STPI IP>
   #+END_SRC
