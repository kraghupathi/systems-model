...
** Functional Requirements
  ...
  1. <<<req1>>> Configuration server will configure all the nodes in the
     cluster including itself and ensures that the configuration is consistent.

  2. <<<req2>>> Configuration server has only one `vlead-jockey' user and
     it has sudo permissions to run all commands.

  3. <<<req3>>> Only authorized VLEAD users can log-in as 'vlead-jockey' user.
...
|-----------------+-----------------------------------------------------------------|
| req1            | At regular intervals, defined firewall rules are applied        |
|                 | thus ensuring consistency                                       |
|-----------------+-----------------------------------------------------------------|
| req2            | A user 'vlead-jockey' is created and sudo permissions are given |
|                 | to it.                                                          |
|-----------------+-----------------------------------------------------------------|
| req3            | public keys of VLEAD users are copied to the authorized keys    |
