...
*** Rule for ongoing connection from other machine 
    ...
    connections. Previous connection would have passed through the
...
*** Rule for incoming SSH connection
    The server allows *only* the IIIT-H network to ssh to it on port 22.
...
** Disable root login via ssh
   Common role of ansible ensures that root login is enabled on all
