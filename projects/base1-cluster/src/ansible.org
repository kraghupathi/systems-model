...
* Introduction
* COMMENT Rename Ansible everywhere with Configuration server.
...
** Functional Requirements 
  ...
  1. <<1>> Ansible server manages itself as well as others listed below:
* COMMENT - Instead of manages itself, Ansible configures all the nodes
in the cluster including itself and ensures that the configuration is
consistent. Rephrase point 3.
...
* COMMENT Enumerate the firewall rules. (1 is not applicable probably)
...
* COMMENT Put design table and define what does manages itself means.
...
* COMMENT This rule is not working, get it checked by Saurabh
...
*** ICMP Rule
* COMMENT Update the test cases for ICMP 
...
********  COMMENT- Rephrase the entire description and heading
 
   - Firewall rules are set by following steps:
      1) Copy the iptables to the ansible machine.
...
* COMMENT- For Configuration server, root is disabled. Rephrase the
  description.
...
* COMMENT - remove equals inside example
   #+BEGIN_EXAMPLE
    - Domain name :: ansible.staging.virtual-labs.ac.in
    - IP Address :: 10.4.14.101
   #+END_EXAMPLE
*** Stop and uninstall httpd if it is present in template used to craete ansible machine
    #+BEGIN_EXAMPLE
    service httpd stop
    yum remove httpd
    #+END_EXAMPLE
...
*** Copy VLEAD users public keys to ansible user's authorized keys
    ...
    #+END_SRC
   Copy all the public keys into /home/ansible/.ssh/authorized_keys 
*** Generate ssh key for ansible user and copy it to it's own authorized_keys(to allow ssh from localhost)
    #+BEGIN_SRC 
    ssh-keygen -t rsa
    cd /home/ansible/.ssh/
...
*** Steps to run the test case
    Test ansible is allowed to ssh from IIIT network. 
   #+BEGIN_SRC 
...
*** Description
    Checking ports
*** Steps to run the test case
    Test ansible machine to block all ports except port 22
    #+BEGIN_SRC 
    $ nmap -v 10.4.14.101
    #+END_SRC
*** Expected result
