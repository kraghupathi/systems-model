...
networks. It acts as a gateway to and for all other servers.This documentation describes
the router server setup on amazon web services using automated scripts. These scripts are
executed directly from the configuration server which is ansible in this case. Router is
configured automatically by the ansible. 
...
** Functional Requirements
  ...
  2) Routes traffic on TCP port 80/443 by forwarding packets to reverse proxy.
  3) Routes traffic on UDP port 53 by forwarding packets to public DNS.
  4) Accept incoming ssh connection from ansible on TCP port 22.
  5) Accept the incoming requests from the lab users via TCP port 80/443
     and UDP port 53.
  6) Accept incoming ssh connection from nagios and management IPs on TCP port 22.

** Security Requirements
  1) TCP port 22 is open for ansible,nagios and management IPs.
  2) TCP port 80,443; UDP port 53 and ICMP (Internet Control Message Protocol)
     are open for all.
  3) Rate limiting on all open ports.
  4) General server hardening rules are enforced by [[./common.org][common]] role.
 
* Design
** Snapshot
#+CAPTION:  Router Diagram
#+LABEL:  fig-router-diagram
#+NAME: fig-router-diagram
** Overall Network diagram of router
The following network diagram represents the structure of the
router. In this diagram only router is the active node. It
shows the connections from/to router to/from other servers such
as DNS,reverse-proxy etc. All the non active nodes are represented using dotted
rectangles. 
