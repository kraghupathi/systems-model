...
the different nodes like DNS (private & public) and reverse-proxy in the network infrastructure designed by us
...
lab users would have to pass through this router server to reach the internal network nodes.The router could be
...
** Security Requirements
  All common security requirements and general server hardening as described in [[file:common.org::*Security Requirements][common role]] are applicable
  to router with an exception that TCP port 80/443 and UDP port 53 are open for all lab users.
...
The description of NAT rules are as follows:\\
...
In NAT rules, the forwarding rules and SNAT rules are described as:\\
...
#The following nat rule forwards the incoming DNS requests via UDP port 53
to public dns
...
#The following nat rule forwards the incoming HTTP request on TCP port 8080 to the ADS server
...
*** Theory
    ...
    Once the query is processed by the Public DNS, it repliesback to
...
*** Observation
    The response recieved form the command-line tools shows that the
    requests were forwarded to the Public DNS from the router to
    process the requests.

*** Conclusion
    The router is forwarding any DNS queries to the Public DNS.
...
*** Theory
    The router on recieving the request on port 80 sends it to the
    reverseproxy. The reverse proxy then forwards the request to the
    required lab requested. To provide a testing environment specific
    for testing the reverse proxy we create a test squid proxy. We
    require a machine which has internet access which can contact the
