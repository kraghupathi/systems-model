...
** Test Case ID :: TC03
*** Objective
    Test to check the tcp port 2222 forwarding to analytics server
*** Apparatus
...
*** Theory
    IIIT Revere proxy server should be able to push the logs to
    AWS analytics server over tcp port 2222 via the AWS router.
*** Procedure 
    After setting up whole cluster on aws including analytics node,
    In Reverse proxy,
    wait for one minute(Some times it may take several minutes to copy
    depends upon file/folder size) to see whether the updated logs are
    getting copied to analytics node.
*** Experiments
    1. Using ansible plabook of analytics node, cron job will be setup
    2. Waited for 10 to 20 minutes to check the logs and awstats are
       copied to analytics server or not
    3. 
