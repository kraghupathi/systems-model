...
# I think private DNS does not resolve the external zones unless we
# use forwarders(if i am wrong correct me), please check that.
...
# this? these?, we can remove we, our in above paragraph.

  Further redundancy using multiple DNS servers has been avoided
  altogether.  This is because if the only available DNS server fails,
...
# replace "the DNS" with "the private DNS"
...
# use some other word instead of "should". What about external clients?
...
# Remove this or replace "this" with "the"
...
# write in separate paragraphs for each folder/file and we are
...
#+END_SRC
    ...
    This template should be placed in the correct location on the
    node. This is done by the following code:
# replace "this" with "the above template" 
#+BEGIN_SRC YAML :tangle roles/named_server/tasks/main.yaml
...
# Remove we. We can write when the service will be called/natified?
...
*** Apply the firewall rules
    ...
    following script.
# read first sentence and make it clear. Use some other word instaed
# of this
...
** Handlers
   ...
   code.
# Remove "should". Use some other word instead of "this"
...
# Please remove "should" in two test cases
** Test case ID: TC01
*** Objective
