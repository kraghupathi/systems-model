...
*** Configure only private dns server
    Configure only Private DNS without installing other clients
...
*** Install clients in  private dns server
    Install ossec_client,rsyslog_client,nagios_client in
    Private-DNS-server.

#+BEGIN_SRC YAML :tangle install_clients_in_private_dns.yaml 
---
- name: This file configures private dns server
  hosts: private_dns
  remote_user: root
