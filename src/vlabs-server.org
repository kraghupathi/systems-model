...
- The following task copies =iptables= template created at [[Firewall%20Rules][Firewall Rules]] to the
...
- name: Get stuff from git
  git: repo=https://github.com/vlead/vlab-web-pages.git dest=/root/vlab_web_pages
...
#+BEGIN_SRC yaml :tangle roles/vlabs_server/tasks/main.yaml
- name: Place copy.sh in the root folder
  copy: src=copy.sh dest=/root/ owner=root group=root mode=744
#+END_SRC
...
** Variables
#+BEGIN_SRC yaml :tangle roles/vlabs_server/vars/main.yaml
---
proxy_env:
...
*** Restart the services
    Restart iptables or httpd if there are any changes in the
    configuration. These handlers are called when associated tasks in
    =tasks/main.yaml= notifies to restart the services.
...
** Main Configuration file
   The main script which includes other tasks/roles mentioned below which runs
   on the vlabs server as follows.
