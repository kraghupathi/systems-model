...
*** Configuring firewall rules
    The following task Copies =iptables= template created at [[Firewall rules]] to
    the server and notifies to restart the iptables service if there are any
...
- name: clone the git repository
  git: clone=yes
       repo=https://github.com/vlead/vlab-web-pages.git
       dest=/root
#+END_SRC
