...
|   `-- main.yml
|-- tasks
|   |-- configure-ads-server.yml
|   |-- install-ads-dependencies.yml
|   `-- main.yml
`-- templates
    `-- ads_iptables
...
- name: Check git is installed
  ...
  register: git_exist
- debug: 
   msg: "git already installed at /usr/local/git. Next task will be skipped" 
  when: git_exist.stat.exists
- name: installing git
  yum: name=git state=present
  environment: "{{proxy_env}}"
  when: git_exist.stat.exists is not defined

- name: setup-ovpl-centos repo exist
  stat: path=~/setup-ovpl-centos
  register: ovpl
- debug:
   msg: "setup-ovpl-centos repo already cloned and next task will be skipped"
  when: ovpl.stat.exists
...
- name: Cloning setup-ovpl-centos repo
  ...
  environment: "{{proxy_env}}"
  when: ovpl.stat.exists is not defined
...
- name: Check tornado-4.3 is already downloaded
  stat: path=~/tornado-4.3.tar.gz
  register: tornado
- debug:
    msg: "tornado is already downloaded, next task will be skipped"
  when: tornado.stat.exists
- name: Downloading python tornado-4.3 file 
  get_url: url=https://pypi.python.org/packages/source/t/tornado/tornado-4.3.tar.gz dest=~/ force=no
  environment: "{{proxy_env}}"
  when: tornado.stat.exists is not defined
...
- name: Getting public key from ADS server from root account
  ...
  register: ssh_keys
- debug: var=ssh_keys.stdout_lines
...
- name: Copying public key of ADS server to Host machine
  ...
  when: hostmachine_password is defined and hostmachine_ip is defined
  register: out
- debug: var=out.stdout_lines

#+END_SRC
*** Configure ADS server with ads_server
    #+BEGIN_SRC yml :tangle ads_server.yaml
---
