...
- debug: 
   msg: "Next task will be skipped if git already installed at /usr/local/git."  
  when: git_exist.stat.exists is defined 
...
- name: Check setup-ovpl-centos repo exist
  stat: path=~/setup-ovpl-centos
  register: ovpl
- debug:
   msg: "Next task will be skipped if setup-ovpl-centos repo is cloned"
  when: ovpl.stat.exists is defined
...
- name: Check tornado-4.3 is already downloaded
  ...
  register: tornado
- debug:
    msg: "Next task will be skipped if tornado is already dowloaded"
  when: tornado.stat.exists is defined
