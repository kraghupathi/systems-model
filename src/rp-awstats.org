...
* Introduction
This Documentation describes the *Reverse Proxy* server setup and
*AWStats* setup for analytics of labs deployed on amazon/base1 using automated
scripts.  +These scripts are executed only from configuration server.+
+This server is managed by [[./config-server.org][configuration server]]+

- Reverse-Proxy :: The client requests comes from router to
                   reverseproxy and the reverse proxy then decides
                   where to send those requests, and returns the
                   content as if it was itself the destination.
- AWStats :: +AWStats is a one of the log analyzer which creates web+
             +related trafic.+ This log analyzer gives us lab user's web
             trafic information such as number of visitors and visits,
             number of pages for a visit, etc. Data is graphically
             presented in easy to read web pages
** COMMENT 
   - It was decided that the description of Reverseproxy must be
     simple and give an outline of its purpose and its service in the
     cluster. Further specific details and how it connects to other
     nodes can be mentioned in the design section.
   - Awstats decsription could be a little more elaborate also
     mentioning why it is being used.
   - reverseproxy should be replaced with reverse proxy.
 
   @Saurabh :: Please give your comments on this !!

* Requirements
  Requirements which are specific to reverseproxy are defined below and
  common requirements are defined in the [[common.org][common role]].
** Functional Requirements 
   - Allow incoming 80 and 443 (HTTP and HTTPS) :: Reverseproxy allows
     incoming connections on tcp ports 80 and 443 to accept the web
     requests coming from the [[./router.org][router]] and internal network. This
     requirement is satisfied by the [[Allow HTTP(80) and
     HTTPS(443) packets][firewall rules]].
   - Allow outgoing 80 (HTTP) ::  Reverseproxy allows outgoing
     connections on tcp port 80 to forward the client requests which
     are coming from the router to the corresponding internal web
     server(lab) and sending reply to the client. This is required
     for yum to update and upgrade the OS. For this requirement look
     at [[Firewall
     rules][OUTPUT rule for 80 in firewall rules section]]
   - Allow outgoing 443 (HTTPS) :: Reverseproxy has to allow outgoing
     connections on tcp port 443 and is required for yum. For this
     requirement look at [[ Firewall rules][OUTPUT rule for 443 in
     firewall rules section]]
   - Store the Analytics :: Reverseproxy will be able to store the
     analytics of each lab using AWStats (Logfile analyzer). All the
     analytics will be stored at =/var/log/httpd/= 

** COMMENT
  We had a discussion among us (model team) to decide whether we need
  to write in points or in a paragraph. Saurabh, we need your
  suggestion on this !!  

  - siva , Yogesh :: This node seems to have more points so we can
  better explain them point wise as compared to paragraphs.
  
  - Soumya :: I felt that the description should be just describing
  basic functionality of reverse proxy. This need not be in detail
  which also explains the network related information. 

  @Saurabh :: Please give your comments on this ?? Also do we require
  communication requirements for this model ??

** Security Requirements 
   Reverseproxy accepts incoming and outgoing connections on tcp ports
   80 and 443 *only* from internal network and from the router.

* Design
** Overall network diagram of reverseproxy
...
#+LABEL:  fig-reverseproxy-diagram
[[./diagrams/Network-Diagram-of-ReverseProxy.png]]

** COMMENT 
   Some descriptions were found appropriate for this section. They can
   be added here. The diagram must be described.
...
** System configuration files 
   ...
   reverseproxy server and AWStats
|------------+--------------------------------------+-------------------+--------------------------------------|
| Service    | File                                 | Associated with   | Description                          |
|------------+--------------------------------------+-------------------+--------------------------------------|
| httpd      | /etc/httpd/conf/httpd.conf           | reverseproxy role | httpd main Configuration             |
|------------+--------------------------------------+-------------------+--------------------------------------|
| httpd      | /etc/httpd/conf.d/virtulhosts.conf   | reverseproxy role | virtual hosts entries  configuration |
|------------+--------------------------------------+-------------------+--------------------------------------|
| AWStats    | /etc/httpd/conf.d/awstats.conf       | reverseproxy role | httpd configuration file for AWStats |
|------------+--------------------------------------+-------------------+--------------------------------------|
| AWStats    | /etc/awstats/awstats.{{labURL}}.conf | reverseproxy role | awstats configuration files for labs |
|------------+--------------------------------------+-------------------+--------------------------------------|
| iptables   | /etc/sysconfig/iptables              | Reverseproxy      | firewall rules                       |
|------------+--------------------------------------+-------------------+--------------------------------------|
| sshd       | /etc/sshd_config                     | Common  Role      | ssh configuration file               |
|------------+--------------------------------------+-------------------+--------------------------------------|
| nameserver | /etc/resolve.conf                    | Common role       | nameservers                          |
|------------+--------------------------------------+-------------------+--------------------------------------|
| IP         | /etc/sysconfig/network-scripts/      | amazon console    | network interfaces  file             |
|------------+--------------------------------------+-------------------+--------------------------------------|
| fail2ban   | /etc/fail2ban/jail.local             | Common Role       | fail2ban configuration file          |
|------------+--------------------------------------+-------------------+--------------------------------------|
| logwatch   | /etc/logwatch/logwatch.conf          | Common Role       | logwatch configuration file          |
|------------+--------------------------------------+-------------------+--------------------------------------|
** COMMENT 
   The common role files are to be removed and the "associated with"
   column would no longer be required.
* Implementation 
- Note  :: In this implementation, variables are used in various
           places using "{}".
- Variables used :: Following variables are used in the ansible
     scripts to configure reverse proxy server and awstats for labs.
   ...
   5. Local Subnet - =local_subnet=

   Values of these variables is fetched from the
   [[file:common_vars.org::*Common Variables][Common vars]] role.
** COMMENT 
   The variables above all belong to common_vars. They can be
   described there and need not be described here.  

   @Saurabh :: While reviewing Private DNS model we decided to have a
   Tasks section (emacs level-2 section) in which there were
   subsections for each task in the yaml file. The templates
   associated with the tasks are mentioned in that subsection
   itself. There is no separate section for templates (level2).  There
   were concerns regarding this format of writing. Handlers is a
   separate section.

   Should all the models follow a similar structure in describing
   their implementations ??\\
   

The implementation of the reverseproxy server and awstats is done with
the following steps.
...
#+END_EXAMPLE
   Here =reverseproxy_server.yaml= file configures reverseproxy_server
   host with reverseproxy_server and other roles. [[Main Configuration
   file][See here for other roles]]
   ...
   Handlers are described in detail at [[Restarting services][handlers]].  The handlers are
   only called when tasks are called as part of the reverseproxy_server
...
** Firewall rules
- Rules which are specific to reverseproxy server are described [[Allow HTTP(80) and HTTPS(443) packets][here]].
- Description of some firewall rules of this reverseproxy and which are common
...
** Install required packages
  ...
  + httpd
  + awstats
  In the following script,use proxy_env variable if network uses any proxy, else comment that or
  remove the line(while working on AWS)
...
#+END_SRC
** Change hostname 
 This task changes the hostname of VM.
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
---
...
Copy =iptables= template which was created at [[Firewall rules]] to reverseproxy.
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
...
** Configuring apache web server
 To support multiple virtual-hosts, enable *NameVirtualHost *:80* in =/etc/httpd/conf/httpd.conf=, and
...
This =virtualhosts.conf= template defines virtual-hosts.  The
template uses "proxy_domains" variable which is defined in common_vars
role, "proxy_domains" variable defines the lab's hostname/FQDN.  
In the following template added default virtual host entry to
reverseproxy server itself to access awstats from browser.

- Note :: If you are working on BASE1, Please change *if condition* of
          the following template according to your domain name of
          reverseproxy which is defined in [[./common_vars.org][common_vars role]]
...
Copy configured [[Allow access to awstats from everywhere][httpd_awstats.conf]] file for accessing
awstats from everywhere to reverseproxy and apply proper ownership, group and mode
permissions to the configuration file. Restart the apache web server if
anything is modified in the configuration file
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
...
This test case is done without internal (private)DNS configuration.
For internal name resolving used =/etc/hosts/= file of the container
***  Objective 
Test to check whether the client requests are going via reverseproxy
and returns the content of particular lab.
*** Apparatus 
            - Three containers ::
...
+ Follow the bellow steps to test the reverseproxy 
  1) Create a container for reverseproxy server
     IP:172.172.172.92
     Run the reverserproxy_server.yaml script to setup reverseproxy server
  ...
  4) create index.html file in each lab container  
     + These index.html files are may be dummy html files just for testing. 
...
Test to check the reverseproxy function using private DNS and Public DN
  - Note :: Here Public DNS not yet configured on AWS. So we are using
            existing public DNS(ns1-pub.virtual-labs.ac.in) which is configured in  BASE2
            machine.
*** Apparatus 
  1) *Router*: 
...
*** Procedure  
  - Assume private DNS and public DNS are configured and added DNS
    entries of each lab properly. 
...
*** Experiment 
  - Added lab1's ip to private DNS by giving the sub-domain as lab1.
  - Added router's public ip to public DNS by giving the sub-domain as lab1.
  - Using browser ::
