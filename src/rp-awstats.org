...
This server is managed by [[./ansible.org][configuration server]].
...
- AWStats :: AWStats is a one of the log analyzer which creates
             advanced web related trafic. This log analyzer gives us
             lab user's web trafic information such as number of
             visitors and visits, number of pages for a visit,
             etc. Data is graphically presented in easy to read web
             pages
* Requirements
...
** Functional Requirements 
  1) Allow incoming connections on tcp ports 80 and 443 from [[.router][router]]
     and from internal network
  2) Allow outgoing connections on tcp port 80
    +  Forwards client requests to corresponding internal web server
       where the lab has been deployed and sending reply to the
       client.
  3) Allow outgoing connections on tcp port 443
    + OUTPUT rule for tcp port 443 is required for yum
...
** Security Requirements 
  1) Accept incoming and outgoing connections on tcp ports 80 and 443 *only* from internal
     network and from the [[.router][router]] 
...
reverseproxy to/from other servers such as [[./router.org][router]],[[./ansible.org][configuration
server]], etc. non active nodes are represented using dotted rectangles.
[[./diagrams/Network-Diagram-of-ReverseProxy-on-AWS.png]]
...
|------+----------+-------------------------------------------+--------------------------------------|
|    1 | httpd    | /etc/httpd/conf/httpd.conf                | httpd main Configuration             |
|------+----------+-------------------------------------------+--------------------------------------|
|    2 | httpd    | /etc/httpd/conf.d/virtual-labs.ac.in.conf | virtual hosts entries  configuration |
|------+----------+-------------------------------------------+--------------------------------------|
|    3 | AWStats  | /etc/httpd/conf.d/awstats.conf            | httpd configuration file for AWStats |
|------+----------+-------------------------------------------+--------------------------------------|
|    4 | AWStats  | /etc/awstats/awstats.{{labURL}}.conf      | awstats configuration files for labs |
|------+----------+-------------------------------------------+--------------------------------------|
|    5 | iptables | /etc/sysconfig/iptables                   | firewall rules                       |
|------+----------+-------------------------------------------+--------------------------------------|

* Implementation 
The implementation of the reverseproxy server and awstats are done
with the following steps.
- Note :: In this implementation, variables are used in different
...
** Firewall rules
- Rules which are specific to reverseproxy server are described [[Allow HTTP(80) packets][here]].
...
*** INPUT firewall rules
 Description of each following firewall rule is defined in
...
- In this model only HTTP packets are accepted. As a future
  requirement, accept rule for incoming connections on tcp port 443 is commented.
...
#+END_SRC
*** FORWARD rules
- Reveseproxy is not responsible for any packet forwarding and hence forwarding rule
...
#+END_SRC
The above rule satisfies functional requirements 2 and 3
*** OUTPUT firewall rules
 Description of each following firewall rule is defined in
 [[./common.org][common role]] 
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/iptables
...
** Install required packages
  ...
  remove the line(while working on amazon)
...
This =virtual-labs.ac.in.conf= template defines virtual-hosts.  The
...
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/virtual-labs.ac.in.conf
...
of each lab which is defined in common role
  + LogFile
  + SiteDomain
  + Hostaliases
...
* Test Cases
** Test Case ID: SSH access only from configuration server
*** Objective 
  To test SSH access from only from ansible and management ips
*** Apparatus 
  1) Configuration server 
  2) Reverseproxy server
  3) A machine with management IP
*** Experiments 
  Reverseproxy server accepts incoming connections on tcp port 22
   only from configuration server.  
**** ssh from config-serevr 
  ssh to reverseproxy from config_server using the bellow command
  #+BEGIN_EXAMPLE
  config-server$ ssh root@<reverseproxy-ip>
  #+END_EXAMPLE
  Result is as follows after logged in 
  #+BEGIN_EXAMPLE 
  Last login: Thu Apr  2 ... other details.....
  reverserproxy$
  #+END_EXAMPLE
**** ssh from management ips
  Reverseproxy server accepts incoming connections on port 22 only
  from management ips ssh from a machine with management ip to
  reverseproxy using the bellow command
  #+BEGIN_SRC example
  management-ips$ ssh root@reverserproxy-ip
  #+END_SRC
  Result
  #+BEGIN_SRC example
   Last login: Thu Apr  2 ---Details of last login---
  reverserproxy$
  #+END_SRC
**** ssh from any other machine
  ssh from any other machine to reverseproxy.
  #+BEGIN_SRC example
  Any-other$ ssh root@reverserproxy-ip
  #+END_SRC
  Result
  #+BEGIN_SRC example
  Permission Denied ......
  #+END_SRC
  That means reverseproxy server is not accepting ssh connection from
  other machine.
*** Observation 
 Reverseproxy is able to accept the incoming connections on port 22
 only from configuration server and management ips.
*** Result 
 Reverseproxy is working properly with SSH configuration.
*** Conclusion 
 SSH and iptables are configured properly. So reverseproxy is
 configured properly and able to accept the connections on port 22
 only from specific ips and configuration server.
...
+ Router accepts HTTP and HTTPS packets from external world (lab
  ...
  reverseproxy sends these packets to internal web servers and returns
  reply to internal network and to the router. Router then gives the
  reply to lab user with the requested web pages.
...
+ Follow the bellow steps to test the reverseproxy 
  ...
  2) create dummy index.html file in each container  
     + These files are dummy html files just for testing. 
     + these files are required to test whether the reverseproxy is forwarding
       the HTTP packets to internal web server based on virtualhost
       entry or not.
  3) Add virtual host entries in reverseproxy in
     ...
     + This virtualhost entry is needed for reverseproxy in order to
       send the incoming HTTP and HTTPS packets to internal web server
       and is reverseproxy server is responsible for where to send
       these packets based on ServerName field in virtualhost entry
  4) Add hostname of each lab in =/etc/hosts= by giving ip of
     lab in reverseproxy and for internal name resolving
  5) Use ping and wget commands to test reverseproxy and do the
...
** Test Case ID : Reverseproxy test using internal(private) DNS and Public DNS
