...
from management ips" and proper apache syntax.
...
Copy [[Allow access to awstats from everywhere][httpd_awstats.conf]] configuration file to reverseproxy and with
proper ownership, group and mode permissions for accessing awstats
from specific ips. The following task is notified to restart apache
web server if there are any changes in the apache configuration.
