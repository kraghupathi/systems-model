...
* Introduction
The document describes the requirements, design and implementation of
the *Reverse Proxy* server setup and *AWStats*. This server will be
configured using ansible scripts/playbooks. 
# comment: Instead of 'The document' it could be 'This document'
# comment: Instead of 'will be configured' it could be 'is configured'
Our cluster consist of many nodes. Reverse proxy is one of the main
node in the cluster. All web related traffic (which are coming from
external world) goes via this node to access the labs.
# comment: Instead of 'web related traffic' it could be 'HTTP requests
# from external world are forwarded to reverse proxy to access the labs'
Configuration server is responsible for configuring the reverse proxy
server by running the reverse_proxy server playbook.
# comment: 'configuration server ...' line is not needed in the
# introduction section, why we are mentioning about configuration
# server here.

Reverse proxy server will be able to collect all the web related traffic
of labs using awstats application. So that analytics can be analyzed.
# comment: Avoid use of 'will'.
# comment: Probably more description about analytics can be added
* Requirements
...
** Functional Requirements
 ...
 2) Allow outgoing connections on tcp port 80 to forward the
    client. And also required for yum. For this requirement look at [[Firewall
     rules][OUTPUT rule for 80 in firewall rules section]]
# comment: Instead of 'For this requirement' we can write 'this
# requirement is fulfilled by [link to the section]'
 3) Allow outgoing connections on tcp port 443 for
    yum. For this requirement look at [[ Firewall rules][OUTPUT rule for 443 in firewall
    rules section]]
 4) Reverse proxy will be able to store the analytics of each lab using
    AWStats (Logfile analyzer).
# comment: Avoid use of 'will'.
** Security Requirements
 1) Accept incoming and outgoing connections on tcp ports 80 and 443
    *only* from internal network and from the router.
* Design
*Design of this node is as follows:*
** Network diagram 
   The following network diagram represents the communication of
   reverse proxy with other nodes in the cluster. 
...
If the requests coming from external world, reverse proxy server
requires router and private DNS(for name resolving) nodes.  If the
requests are coming from internal network, reverse proxy server needs
private DNS only for name resolving.
# comment: Description is not clear. Reverseproxy does not need
# router, router forwards request to reverseproxy. 

This node has been configured using ansible playbook of reverse proxy
from config_server, So that it is able to contact other
labs(containers) using private DNS. After contacting lab sources,
reverse proxy server sends response back to the lab requester (via the
router if user requests lab from external world).
# comment: "So that it is ..." is not clear. Information is repeated
# same content is there in previous paragraph.


Firewall rules are configured as per the security requirements, such
that this node can be secured as much as possible. Other requirements
such as fail2ban,logwatch etc., are configured by common role.

In this cluster awstats configuration also plays an important role in
the reverse proxy server. Awstats configuration is for storing web
related traffic of each lab in separate file. So that we can have the
history of the analytics stored in reverse proxy. AWStats is log
analyzer which gives us web related traffic. This log analyzer gives us
lab user's web traffic information such as number of visitors and
visits, number of pages for a visit, etc. Data is graphically
presented in easy to read web pages

# comment: "In this cluster awstats configuration also plays an
# important role in the reverseproxy server" - This information seems
# to be redundent. The description can be improved further.
...
- Note :: In this implementation, variables are used in various places
          ...
          [[common_vars.org][common_vars role]].
# comment: Combine different sections into tasks and handlers section.
...
#+END_EXAMPLE
   ...
   Handlers are described in detail at [[Restarting services][handlers]]. The handlers are only
   ...
   tasks which are need for setting up reverseproxy server and
   awstats.

# comment: replace 'need' with 'needed'
   ...
   is created at [[Default template to configure AWStats][template]] and task is implemented [[Configure AWStats for each lab][here]]
   
# comment: 'is created at template' could be better replaced to 'is
# created at template section'
...
- Description of some firewall rules of this reverse proxy and which are common
  across all the systems are described in [[./common.org][common role]].  

# comment: The above sentence can be better written as 'Description of
# some firewall rules of the reverse proxy, which are common across
# all the nodes in the cluster are described in [section]'
...
- The firewall rule accepts HTTP(80) and HTTPS(443) packets from internal network and
from the [[.router.org][router]]. 
- In this model, only HTTP packets are accepted. As a future
  requirement, accept rule for incoming connections on tcp port 443 is
  commented.
#+BEGIN_SRC example
# Allow incoming http packets from internal network
-A INPUT -s {{local_subnet}} -p tcp -m tcp --dport 80 -j ACCEPT
...
** Install required packages
  Install required packages for setting up reverse proxy and awstats.

# comment: Following packages are installed for setting up reverse
# proxy. Above line can be removed.
  ...
  + awstats
  + mod_ssl
  In the following script,use proxy_env variable if network uses any
  proxy, else comment that or remove the line(while working on AWS)
- Note :: Install "mod_ssl" for HTTPS access. Currently HTTPS access
          is not enabled. Enable it if it has a valid certificate and
...
#+END_SRC
** Change hostname 
 This task changes the hostname of instance.

# comment: It does not change the hostname rather it sets the hostname

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
---
- name: Changing the hostname
  hostname: name=reverseproxy.virtual-vlabs.ac.in
#+END_SRC
** Configuring firewall rules 
Copy =iptables= template which was created at [[Firewall rules]] to
reverseproxy.

# comment: Copy =iptables= tempalte created at [section] to reverseproxy.
...
** Configuring apache web server
 To support multiple virtual-hosts, enable *NameVirtualHost *:80* in
 =/etc/httpd/conf/httpd.conf=, and restart the apache web-server

# comment: Do not use imperative sentences. Re-write it as "To support
# multiple virtual-hosts *NameVirtualHost *:80 in
# =/etc/httpd/conf/httpd.conf= is enabled and
# apahce web-server is restarted"
...
This =virtualhosts.conf= template defines virtual-hosts.  The template
uses "proxy_domains" variable which is defined in common_vars role,
"proxy_domains" variable defines the lab's hostname/FQDN.  In the
following template, added reverse proxy server as default virtual host
entry to access the awstats from browser.

# comment: "In the following template, default virtual host entry is
# added as reverse proxy server, to access the awstats from browser."

- Note :: If you are working on BASE1/BASE4, Please change *if condition* of
          the following template according to your domain name of
          reverse proxy which is defined in [[./common_vars.org][common_vars role]]
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/virtualhosts.conf

{% for domain in proxy_domains %}
{% if domain.domain == "reverseproxy.virtual-labs.ac.in" %}
<Virtualhost *:80>
ServerAdmin engg@vlabs.ac.in
ServerName {{domain.domain}} 
ServerAlias {{domain.alias}}
ServerAlias {{domain.alias1}}
...
Copy =virtualhosts.conf= template to reverseproxy server's apache
configuration file  =/etc/httpd/conf.d/virtualhosts.conf=  and
restart the apache web server if configuration file is modified.

# comment: Copy =virtualhosts.conf= template to reverseproxy server's
# apache configuration file
# =/etc/httpd/conf.d/virtualhosts.conf=. Apache web service is
# restarted if there is any change in the configuration. 
...
In this following template we modified three fields in order to
configure AWStats for each lab. ={item}= variable used as domain name
of each lab which is defined in [[./common.org][common role]]
  + LogFile
  + SiteDomain
  + Hostaliases


# comment: In the following template three fields are modified in
# order to configure AWStats for each lab. ={item}= variable used as
# domain name of each lab is defined in [common role].
...
Copy default [[AWStats configuration][awstats.reverseproxy.conf]] template to reverseproxy.
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
...
To allow access to awstats from management ips , configure apache
server configuration file =/etc/httpd/conf.d/awstats.conf= by adding
"allow from management ips".

# comment: by adding "allow from management ips" --> by adding "allow
# from" and proper apahce syntax.
...
Copy [[Allow access to awstats from everywhere][httpd_awstats.conf]] configuration file for accessing awstats from
everywhere to reverseproxy and apply proper ownership, group and mode
permissions. Restart the apache web server if anything is modified in
the configuration file.

# comment: Copy httpd_awstats.conf configuration file to reverseproxy
# with proper ownership, group and mode permissions for accessing
# awstats from everywhere.
...
#+END_SRC
** Restarting services
Restarting the services (httpd and iptables) if anything is modified in
configuration file.

# comment: Restart the services ...
...
This is the main script which includes other tasks/roles mentioned
below which we run on reverseproxy server.

# comment: This is the main script which includes other tasks/roles
# mentioned below which is run on the reverseproxy server.

 1) [[./common.org][common]]
 2) [[./ossec-client.org][ossec_client]]
 3) [[./common-vars.org][common_vars]]
 4) [[./rsyslog-client.org][rsyslog-client]]
 5) [[./nagios-client.org][nagios-client]]

#+BEGIN_SRC yaml :tangle reverseproxy-server.yaml
---
...
internal name resolving use =/etc/hosts/= file of the container

# comment: This test is done without internal (private) DNS
# configuration. For internal name resovling =/etc/hosts= file is used.

*** Objective
Test to check whether the client requests are going via reverseproxy
and returns the content of a particular lab.

# comment: Test to check whether the client requests are going via
...
*** Apparatus 
            - Three containers ::
              ...
              2) lab1 container 
              3) lab2 container 
            - /etc/hosts of reverseproxy  ::
              1) To add hostnames of labs. These labs points to
...
# comment: /etc/hosts is not an apparatus. It is present inside the
# server itself.

*** Procedure 
+ Follow the bellow steps to test the reverseproxy 
  1) Create a container for reverseproxy server
     IP:172.172.172.92
     Run the reverserproxy_server.yaml script to setup reverseproxy
     server

# comment: This step is not needed. We are testing the existing
# reverseproxy server we are not creating a new reverseproxy server
# for testing.
  ...
  2) Create two new containers lab1 and lab2 and install dependencies
     ...
     + Dependencies ::
         a) httpd
         b) php(if needed)
  3) Add virtual host entries in reverseproxy.
     + Adding virtual host entries is taken care by ansible scripts
       of reverseproxy_server.
     + This virtualhost entry is needed for reverseproxy in order to
       send the incoming HTTP and HTTPS packets to internal web
       servers and reverseproxy is responsible for where to send these
       packets based on ServerName field in virtualhost entry

# comment: This step might also not be needed as we are using existing
# reverseproxy server. We can provide example of existing virtualhost.

  4) create index.html file in each lab container  
     + These index.html files may be dummy html files just for testing. 
     + these files are required to test whether the reverseproxy is forwarding
       the HTTP packets to internal web server or not, based on virtualhost
       entry.
...
*** Observation 
  - Absence of --no-proxy (if network uses any proxy server), gives
    you following error.

# comment: gives you following error -> throws the following error.
...
# comment: Now browse the Lab URL from browser.

  - Router accepts HTTP and HTTPS packets from external world (lab
    user)and [[./router.org][router]] forwards the requests to reverseproxy server since
    router has the forwarding capacity. And then reverseproxy accepts
    the HTTP and HTTPS packets from [[./router.org][router]] and from internal network,
    based on ServerName directive in virtualhost, reverseproxy sends
    these packets to internal web servers and returns reply to
    internal network and to the router. Router then gives the reply to
    lab user with the requested web pages.

# comment: Instead of 'capacity' it is better to use functionality
# Do not make 'router' word as link.
...
*** Observation 
 ...
 server.
*** Result 
Reverseproxy working
*** Conclusion 
Reverseproxy setup properly, we are able to access the lab using
domain name of the lab and logs are being stored for analytics.

# comment: Reverseproxy is setup properly, we are able to access the
# lab using domain name of the lab and logs are being stored for analytics.

** Test Case ID : AWStats test
*** Objective 
Test to check the AWStats for virtual hosts
*** Apparatus 
     1) Default virtual-host entry for reverseproxy to serve awstats
        from the browser
     2) Access log file for each lab to check the accepted log messages 
     3) Error log file for each lab to check the error log messages.
     4) A workstation to access the analytics from the browser.

# comment: Files are not the apparatus. Only machines are considered
# as the apparatus.

*** Procedure  
     - Run the reverseproxy_server ansible scripts to setup
       virtualhost entries and awstats. 
...
# comment: First step will not be needed when we use existing
# reverseproxy server. We don't have to run any ansible scripts.
