...
communication with other services and servers. In this diagram only reverseproxy is active node. It
shows the connections from/to reverseproxy to/from other servers such
as [[./router.org][router]],[[./ansible.org][configuration server]], etc. non active nodes are represented using dotted
rectangles.  
