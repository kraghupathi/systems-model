...
- Router accepts HTTP and HTTPS packets from external world (lab
  ...
  reply to internal network and to the router. Router is then gives
  the reply to lab user with the requested web pages.
...
- Follow the bellow steps to test the reverseproxy 
  ...
  3) Add virtual host entries in reverseproxy in
    ...
    + This virtualhost entry is needed for reverseproxy in order to
      ...
      these packets based on NameServer filed in virtualhost entry
...
+ Experiments ::
  ...
  - From command line ::
    ...
    #+END_SRC
    + Add hostnames of the labs to =/etc/hosts= to my PC
    #+BEGIN_SRC 
