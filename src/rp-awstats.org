#+Title: Reverse Proxy server and AWStats 
#+PROPERTY: session *scratch*
#+PROPERTY: results output
#+PROPERTY: exports code
#+SETUPFILE: org-templates/level-0.org
#+DESCRIPTION: Reverse Proxy Server - Design and Documentation

* Introduction
This Documentation describes the Reverse-proxy server setup and
AWStats setup for analytics of labs deployed on amazon using automated
scripts.  These scripts are executed only from configuration server.
This server is managed by configuration server.

- Reverse-Proxy :: The client makes requests for content in
                   the name-space of the reverse proxy. The reverse
                   proxy then decides where to send those requests,
                   and returns the content as if it was itself the
                   origin.
- AWStats :: AWStats is powerful log analyzer which creates advanced
             web, ftp, mail and streaming server statistics reports
             based server logs.  Data is graphically presented in easy
             to read web pages
	     
* Requirements
The requirements to configure reverseproxy server are as follows.
** Functional Requirements 
  1) Accept ssh connection from configuration server and
     management ips on tcp
     port 22.
  2) Accept incoming and outgoing connections on tcp port 80
  3) Accept outgoing connections on udp port 53
  4) Forward client requests to corresponding lab where
     the lab has been deployed and sending reply to the client.
  5) Store the analytics of each lab using AWStats (Logfile analyzer) 
  6) Allow outgoing tcp port 25  to logwatch service for sending logwatch emails
  7) Allow outgoing 1514 udp packets to OSSEC server
  8) Allow incoming connections on tcp port  5666 from nagios
     monitoring server for nrpe queries
  9) Allow the outgoing connections on udp port 514 to rsyslog
     server 
  10) Allow the incoming connections on udp ports 161 and 162 from
     cacti server
  11) Allow ssh status check from nagios
      + Nagios monitoring server monitors whether the ssh service
	is running or not. Just checking the status of the services in
	reverseproxy. Nagios does not have ssh access to reverseproxy 
  
*** TODO 
 - router,configuration links
 - rsyslog, OSSEC server links output rules 
** Security Requirements 
  1) Accept ssh connection *only* from configuration server
     and from management ips
     + Only configuration server and management ips can be able ssh to reverse-proxy to
       run the scripts and bringing up the reverseproxy server and
       awstats 
  2) Rate limiting to avoid DoS(Denial of Service) attacks 
  3) Disabling password based authentication for root account
  4) allow incoming and outgoing http packets from internal
     network and from the router 
* Design of Reverse-Proxy 
...
The following network diagram represents the structure of the
reverseproxy. In this diagram only reverseproxy is active node. It
shows the connections from/to reverseproxy to/from other services such
as cacti, nagios, etc. non active nodes are represented using dotted
rectangles.  
[[./build/diagrams/Network-Diagram-of-ReverseProxy-on-AWS.png]]

** Incoming and outgoing connections of reverseproxy

The following diagram represents the incoming and outgoing connections
of reverseproxy server.

[[./build/diagrams/Incoming-and-Outgoing-connections-of-Reverseproxy.png]]
** System files 
|------+----------+-------------------------------------------+--------------------------------------|
| S.no | Service  | File                                      | File Type                            |
|------+----------+-------------------------------------------+--------------------------------------|
|    1 | httpd    | /etc/httpd/conf/httpd.conf                | Httpd Configuration                  |
|------+----------+-------------------------------------------+--------------------------------------|
|    2 | httpd    | /etc/httpd/conf.d/virtual-labs.ac.in.conf | virtual hosts configuration          |
|------+----------+-------------------------------------------+--------------------------------------|
|    3 | AWStats  | /etc/awstats/awstats.{{domain}}.conf      | awstats configuration files for labs |
|------+----------+-------------------------------------------+--------------------------------------|
|    4 | AWStats  | /etc/httpd/conf.d/awstats.conf            | awstats configuration                |
|------+----------+-------------------------------------------+--------------------------------------|
|    5 | iptables | /etc/sysconfig/iptables                   | firewall rules                       |
|------+----------+-------------------------------------------+--------------------------------------|
...
**** Default filter rules
  Default policy for input, forward and output is set to be as
  ACCEPT. If packet does not match any rule then the default action is
  applied to the packet.  Packets counters are set to [0:0].  First
  counter represents the number of packets that matched the rule for
  the chain, and second counter represents the total size of the
  packets that matched the rule.
  
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
*filter
:INPUT ACCEPT [0:0]
:FORWARD ACCEPT [0:0]
:OUTPUT ACCEPT [0:0]
...
- Allow internal communication between services running within the reverse proxy, over
  loopback interface. Input destination interface is specified to avoid any security
  breach.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables 
...
If user tries to accept more than 20 connections per 30 seconds, the
...
#+END_SRC
The above rule satisfies the security requirements 2
**** Rate Limit on ICMP 
...
Allows ongoing connections which are related and established connections
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow ongoing connections
-A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
#+END_SRC
**** SSH access from management IPs
Allow incoming SSH connections from management IPs.  Hopefully
fail2ban will take care of brute-force attacks from management
IPs. Please look at common role tasks for fail2ban
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
...
{% endfor %}
#+END_SRC
The above rule satisfies the security requirements 5
**** SSH access from configuration server
Firewall rule for Accepting ssh connection only from configuration server
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
...
{% endfor %}
#+END_SRC
The above rule satisfies the Security Requirements 1
**** SSH status check from nagios server
Allow nagios server to monitor ssh service status(up/down). Nagios
server does not have ssh access to reverseproxy.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
-A INPUT -p tcp -m tcp -s {{nagios_server_ip}} --dport 22 -j ACCEPT
#+END_SRC
The above rule satisfies the functional requirements 11
**** Allow HTTP(80) packets
This firewall rule allows HTTP(80) packets from internal network and
from the router
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
# Allow incoming http packets from internal network and router
-A INPUT -s {{internal_vpc}} -p tcp -m tcp --dport 80 -j ACCEPT
-A INPUT -s {{router_ip}} -p tcp -m tcp --dport 80 -j ACCEPT
#+END_SRC
The above rule satisfies the security requirements 4
**** NRPE queries from nagios
This rule allows incoming connections on tcp port 5666 from nagios server 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow incoming NRPE queries for nagios from nagios servers
-A INPUT -m state --state NEW -p tcp -m tcp --dport 5666 -j ACCEPT
#+END_SRC
The above firewall rule satisfies the functional requirements 8

**** SNMP queries from cacti 
...
#+END_SRC
The above firewall rule satisfies the functional requirements 10
**** Logging blocked inputs 
This firewall rule logs all other input connections made on denied ports
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Log all other "blocked_input_" attempts with rate limiting
-A INPUT -m state --state NEW -m limit --limit 2/min -j LOG --log-prefix "blocked_input_"
#+END_SRC
The above firewall rule satisfies the security requirements 2
**** ICMP rule
...
*** FORWARD Rules
**** Accept forward rule
This firwal rule enables packet forwarding 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
...
**** Outgoing connections to localhost
 Allow internal communication between services running within the
 router, over loopback interface. Output destination interface is also specified
...
#+END_SRC
**** Ongoing Connections
 Allow reverseproxy to continue already related and established
 connections. Previous connection must have passed through the
 firewall and if they got established, connection is allowed to
 continue.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow ongoing connections
...
#+END_SRC
**** DNS queries
  This firewall rule allows  dns queries for name resolving 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow DNS queries
-A OUTPUT -p udp -m udp --dport 53 -j ACCEPT
#+END_SRC
The above firewall rule satisfies the functional requirements 3
**** Accept logwatch packets
This Firewall rule allows OUTPUT rule for sending logwatch emails
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow server to send emails.  Required for sending logwatch emails
-A OUTPUT -p tcp -m tcp --dport 25 -j ACCEPT
#+END_SRC
The above rule statisfies the functional requirements 6
**** Accept http packets
The following firewall rule accepts http packets coming from internel
network and router and then forwards the requests to internal web
servers.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
...
#+END_SRC
The above rule satisfies functional requirements 2
**** outgoing PING requests
Allows outgoing ping requests.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow outgoing ping requests
-A OUTPUT -p icmp --icmp-type echo-request -j ACCEPT
#+END_SRC
**** Allowing rsyslog 
This rule allows outgoing connections on udp port 514 to rsyslog server 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow outgoing connections to rsyslog server
-A OUTPUT -p udp -m udp --dport 514 -j ACCEPT
#+END_SRC
The above rule satisfies functional requirements 9
**** Outgoing connections to OSSEC server
Allowing outgoing connections to OSSEC server on udp port 1514
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Allow outgoing connections to OSSEC server
-A OUTPUT -p udp -m udp --dport 1514 -j ACCEPT
#+END_SRC
The above rule satisfies functional requirements 7
**** Log all other denied attempts
This firewall rule logs all other output connections made on denied ports
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Log all other "blocked_output_" attempts
-A OUTPUT -m state --state NEW -m limit --limit 2/min -j LOG --log-prefix "blocked_output_"
#+END_SRC
The above rule satisfies the security requirements 2
**** ICMP rule with proper error reply
 Output ICMP rule with proper error reply 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/iptables
#Reply with proper ICMP error message and reject the connection
-A OUTPUT -j REJECT --reject-with icmp-host-prohibited
#+END_SRC
** Installing required packages
  Installing required packages for setting up reverse proxy and
  awstats 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/tasks/main.yaml
---
...
Copy template =iptables= which was created at [[firewalls]] to reverseproxy.
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/tasks/main.yaml
...
- name: Configure apache to support multiple virtualhosts
  ...
  notify:
    - restart httpd
#+END_SRC
** Creating template for virtualhosts
This template =virtual-labs.ac.in.conf= defines virtual-hosts. 
The template uses "proxy_domains" variable which is defined in
common_vars role , the "proxy_domains" variable defines the lab's hostname/FQDN

#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/virtual-labs.ac.in.conf
{% for domain in proxy_domains %}
<VirtualHost *:80>
ServerAdmin engg@virtual-labs.ac.in
...
Copy =virtual-labs.ac.in.conf= template to reverseproxy server's apache
configuration file  =/etc/httpd/conf.d/virtual-labs.ac.in.conf=  and
restart the apache web server if configuration file is modified.
 #+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/tasks/main.yaml
 - name: Configure virtual-host for all proxy_domains
   template: src=virtual-labs.ac.in.conf dest=/etc/httpd/conf.d/virtual-labs.ac.in.conf
   notify:
    - restart httpd
 #+END_SRC
** <<awstats>> AWStats configuration
...
of each lab which is called by *with-items* with *proxy_domains* in
main task file =taks/main.yaml=. 
  + LogFile
  + SiteDomain
  + Hostaliases

The actual default template which we need to copy to reverseproxy is 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/templates/awstats.reverseproxy.conf
...
Copy default template [[awstats][awstats.reverseproxy.conf]] to reverseproxy 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/tasks/main.yaml
...
*** Allow access to awstats from everywhere
To allow access to awstats from everywhere , configure apache server's
configuration file =/etc/httpd/conf.d/awstats.conf= by adding *Allow
from all*
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/files/httpd_awstats.conf
#
...
#+END_SRC
*** Apply allow access to awstats from everywhere
Copy configured =httpd_awstats.conf= file for accessing
...
#+BEGIN_SRC yaml :tangle build/reverseproxy_server.yaml
---
...
configuration file 
#+BEGIN_SRC yaml :tangle build/roles/reverseproxy_server/handlers/main.yaml 
---
- name: restart httpd
...
* Provisioning for AWS cluster
  Reverse-Proxy is setup by the Configuration server. Machine configuration is described
  below.

** Reverse-Proxy machine configuration:
...
** Reverse-Proxy machine is created using:
   AWS console by giving private ip
** Domain name and IP address of Reverse-Proxy machine:
    - Domain name :: reverseproxy.virtual-labs.ac.in
    - IP Address :: <reverseproxy_ip>

** Access to Reverse-Proxy Machine
- Only configuration server's public key is to be placed as authorized key in the reverseproxy
     
* Test Cases
** Test Case ID: TC01 
+  Objective :: To test whether the client requests are going via
                reverseproxy and returns the content of particular
                lab.
+ Apparatus :: 
            - Three containers ::
            ...
            - /etc/hosts of reverseproxy  ::
              1) To add hostnames of two lab, so these names are used
                 in virtual-hosts. and
	      2) For internal name resolving
            - PC/extra container ::
              1) To add hostname of the lab to =/etc/hosts= file 
             
              
+ Procedure ::
  1) Create two new containers and install dependencies 
     + Dependencies ::
         a) http
         b) php(if needed)
  2) create dummy index.html file in each container  
  3) Add virtual host entries in reverseproxy in
     =/etc/httpd/conf.d/virtual-labs.ac.in= of two labs
  4) Add hostname of each lab in =/etc/hosts= by giving ip of
     lab in reverseproxy and for internal name resolving
  5) use ping and wget commands to test reverseproxy
+ Experiments ::
  - From command line ::
   To test whether the reverseproxy is working or not from command
       line , follow the steps shown below
    + Add hostnames to =/etc/hosts= to my PC
    #+BEGIN_SRC 
    reverseproxy_ip lab1.virtual-labs.ac.in
    reverseproxy_ip lab2.virtual-labs.ac.in
    #+END_SRC
   ...
   + ping from reverseproxy container ::
      #+BEGIN_SRC 
   ...
   + ping from PC ::
    ...
    #+END_SRC
 - From Browser ::
   + Assume you have added hostnames to =/etc/hosts= in your PC like
     below
    #+BEGIN_SRC 
    reverseproxy_ip lab1.virtual-labs.ac.in
    reverseproxy_ip lab2.virtual-labs.ac.in
    #+END_SRC
   + Set no proxy for *virtual-labs.ac.in in the browser 
   + Browse the lab URLs http://lab1.virtual-labs.ac.in and
     http://lab2.virtual-labs.ac.in from your browser and you may get
     the following result in log file at
     =/var/log/httpd/lab1.virtual-labs.ac.in-access-log=
    #+BEGIN_SRC 
...
+ Result :: Reverse Proxy is working
+ Observation ::
...
+ Apparatus :: 
     ...
     3) Error log file for each lab to check the error log messages while
        trying to access web pages.
     4) Access awstats from everywhere
+ Procedure :: 
     - Add virtual host entries in
       =/etc/httpd/conf.d/virtual-labs.ac.in= along with the following
       directives
       #+BEGIN_SRC 
       CustomLog logs/lab1.virtual-labs.ac.in-access-log combined
       ErrorLog logs/lab1.virtual-labs.ac.in-error-log
       #+END_SRC
       for each lab
     - Restart the apache webserver
     - run the following command to update analytics of each lab
       #+BEGIN_SRC 
       $perl /usr/share/awstats/tools/awstats_updateall.pl now
       or
       $perl /usr/share/awstats/wwwroot/cgi-bin/awstats.pl -config=<labURL> -update
       #+END_SRC

+ Experiments ::
    - From Command Line ::
       Ran following command from my PC
      #+BEGIN_SRC 
      wget --no-proxy  lab1.virtual-labs.ac.in
      #+END_SRC
      + Result on reverseproxy  ::
	 Watched log file by running 
         #+BEGIN_SRC 
    ...
    - From browser ::   
      +	Browse the lab URL from any browser and see the log file by
        running following command in reverseproxy server
        #+BEGIN_SRC 
	$tail -f /var/log/httpd/labURL.access-log
	#+END_SRC
...
+ Result :: Awstats are running fine
+ Conclusion ::
     Awstats ( analytics of each lab ) are storing and able to access
               the analytics from browser
