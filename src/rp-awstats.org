...
- name: Configure apache ssl.conf to listen on 443
  lineinfile: line="Listen *:443 http" regexp="^Listen 443" dest=/etc/httpd/conf.d/ssl.conf state=present
  tags: 
   - https
...
{% elif domain.domain == "bandwidthd.virtual-labs.ac.in" %}
<Virtualhost *:80>
ServerAdmin engg@vlabs.ac.in
ServerName {{domain.domain}}
ServerAlias {{domain.alias}}
