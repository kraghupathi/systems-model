...
|------+----------+-------------------------------------------+--------------------------------------|
|    3 | AWStats  | /etc/httpd/conf.d/awstats.conf            | httpd configuration file of AWStats  |
|------+----------+-------------------------------------------+--------------------------------------|
...
- Description of each firewall rule of this reverseproxy and which are common
  across all the systems are defined in [[.common.org][common role]].  
...
+ Objective :: To test SSH access from only from ansible and
                management ips
+ Apparatus :: 
	       1) Configuration server 
               2) Reverseproxy server
+ Experiments ::  Reverseproxy server accepts incoming connections on
                 tcp port 22 only from configuration server.
  ...
  *ssh from any other machine*
  ssh from any other machine to reverseproxy is to be denied.
  #+BEGIN_SRC example
  ...
  Permission Denied ......
  #+END_SRC
+ Observation :: Reverseproxy is able to accept the incoming
                 connections on port 22 only from configuration
                 server and management ips.
+ Result :: Reverseproxy is working properly with SSH configuration.
+ Conclusion :: SSH and iptables are configured properly. So
                Reverseproxy is configured properly and able to accept
                the connections on port 22 only from specific ips and
...
+ Observation :: Analytics are being stored 
...
+ Objective :: To test reverseproxy using private DNS and Public DN
...
+ Experiment ::
  - Assume we have added lab1's ip to private DNS by giving the
    sub-domain as lab1.
  - Using browser ::
...
+ Observation :: Displaying lab's content and logs are being stored at
                 reverseproxy server.
+ Result :: Reverseproxy working
+ Conclusion :: Reverseproxy setup properly, we are able to access the
                lab using domain name of the lab and logs are being
                stored. 
