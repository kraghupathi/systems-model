...
**** Generating and copying ssh keypair for rsyncing logs, stats to analytics server
     Here we are setting the =cron= job required to =rsync= AWstats,
     Apache logs at regular interval, for this process to be automatic
     we generate a password less SSH key using which this node
     connects to the analytics server.
...
- name: Rsync the Apache log files to analytics server
  cron:
        ...
        hour=01
        state=present
        job="rsync -arR /var/log/httpd root@10.100.1.12:/root/aws/"
