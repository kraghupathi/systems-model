...
server's configuration file =/etc/httpd/conf.d/awstats.conf= by adding
"allow from management ips". The following template uses for-loop for
management ips.
...
>>>>>>> 927fd00832d4205e71d8b04c113380e303603cb6
...
+ Follow the bellow steps to test the reverseproxy 
  1) Create a container for reverseproxy server
     IP:172.172.172.92
     Run the reverserproxy_server.yml script to setup reverseproxy server
  ...
  3) Add virtual host entries in reverseproxy in
     =/etc/httpd/conf.d/virtualhosts.conf= of two labs
  ...
  4) create dummy index.html file in each container  
...
*** Experiments 
    - From Command Line ::
	 ...
	 #+END_SRC
         and you may get the following result
         #+BEGIN_SRC example
