...
the *Reverse Proxy* server setup and *AWStats*. This server is
configured using ansible scripts/playbooks.  

Our cluster consist of many nodes. Reverse proxy is one of the main
node in the cluster. All hhtp and https requests external world are
...
etc. Aanalytics can be viewed on terminal and also in the browser.
...
** Functional Requirements
 ...
 2) Allow outgoing connections on tcp port 80 to forward the
    client. And also required for yum. This requirement fulfilled by [[Firewall
...
from the config_server. If the requests coming from external world,
router accepts the requests and forwards them to reverse proxy server.
Then the reverse proxy decides where to send those requests based on
virtual host entries in the reverse proxy server's apache
configuration. In this case both public DNS and private DNS are needed
for name resolving.

So reverse proxy contacts other labs(containers) using private
DNS. After contacting lab sources, reverse proxy server sends response
back to the lab requester (via the router if user requests lab from
external world).
...
AWStats configuration is also plays an important role in the reverse
proxy server. Awstats configuration is for storing web(http and https)
related traffic of each lab in separate file. So that history of the
analytics available in the reverse proxy server.
...
- Description of some firewall rules of the reverse proxy,which are
  common across all the nodes in the cluster are described in [[./common.org][common
  role]].
...
Copy =iptables= template created at [[Firewall rules section]] to
reverseproxy.
...
**** Enable http virtual-hosts
 ...
 =/etc/httpd/conf/httpd.conf= is enabled.
...
This =virtualhosts.conf= template defines http and https
virtual-hosts.  The template uses "proxy_domains" variable which is
defined in common_vars role. In the following template, default
...
of each lab is defined in [[./common.org][common role]]
  + LogFile
  + SiteDomain
  + Hostaliases
...
below which runs on the reverse proxy server.
