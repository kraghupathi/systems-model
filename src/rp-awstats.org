#+Title: Reverse Proxy and AWStats 
#+PROPERTY: session *scratch*
#+PROPERTY: results output
#+PROPERTY: exports code
#+OPTIONS: ^:nil
#+SETUPFILE: org-templates/level-0.org
#+DESCRIPTION: Reverse Proxy Model - Design, Implementation and Documentation


* Introduction
This document describes the requirements, design and implementation of
the *Reverse Proxy* server setup and *AWStats*. The server is
configured using ansible scripts/playbooks.  

Our cluster consist of many nodes. Reverse proxy is one of the main
node in the cluster. All http and https requests external world are
forwarded to reverse proxy to access the labs.  

Log analyzer (AWStats) gives us lab user's web trafic information such
as number of visitors and visits, number of pages for a visit,
etc. Analytics can be viewed on terminal and also in the browser.

* Requirements
  Requirements which are specific to reverse proxy are defined below and
  common requirements are defined in the [[common.org][common role]].
** Functional Requirements
 1) Allow incoming connections on tcp ports 80 and 443
    to accept the web requests coming from the [[./router.org][router]].
 2) Allow outgoing connections on tcp port 80 to forward the
    client. And also required for yum. This requirement is fulfilled by [[Firewall
     rules][OUTPUT rule for 80 in firewall rules section]]
 3) Allow outgoing connections on tcp port 443 for yum. This
    requirement is fulfilled by [[ Firewall rules][OUTPUT rule for 443 in firewall rules
    section]]
 4) Stores analytics of each lab using AWStats (Logfile analyzer).
 5) Revers proxy uses router IP as default gateway to reach the
    external world.
 6) Forward the virtual hosts Custom(access), Error logs to =rsyslog=
    node.
** Security Requirements
  1) Accept incoming connections on tcp ports 80 and 443 from
     anywhere.
  2) Allow outgoing connections on port 80 to local network only.
 
* Design
   Reverse proxy server accepts the http and https requests(coming from
   external world)from the router. Those requests are redirected to
   reverse proxy server and the reverse proxy then decides where to send
   those requests, and returns the content as if it was itself the
   destination.
   
   Reverse proxy server is reachable(PINGable) from other nodes as well.
   
   This node has been configured using reverse proxy ansible role from
   the config-server. If the requests coming from external world,
   router accepts the requests and forwards them to reverse proxy
   server.  Then the reverse proxy decides where to send those
   requests based on virtual host entries in the reverse proxy
   server's apache configuration. In this case both public DNS and
   private DNS are needed for name resolving.
   
   So reverse proxy contacts other labs(containers) using private
   DNS. After contacting lab servers, reverse proxy server sends
   response back to the lab requester(via the router.  Note that only
   external requests are served through reverseproxy.  If for some
   strange reason lab1 need to access lab2 then they can contact each
   other using private-dns without depending upon reverseproxy.

   Firewall rules are configured as per the security
   requirements. Other requirements such as fail2ban,logwatch etc.,
   are configured by common role.
   
   AWStats configuration is also plays an important role in the
   reverse proxy server.  Awstats configuration is for processing web
   (http and https) usage logs of each lab such that analytics of
   client / user history are available in the reverse proxy server.

** Network diagram 
   The following network diagram represents the communication between
   reverse and the other nodes in the cluster.
   #+CAPTION:  Reverse-Proxy Network Diagram
   #+LABEL:  fig-reverse proxy-diagram
...
** System configuration files 
   ...
   reverse proxy server and AWStats

   |----------+--------------------------------------+--------------------------------------|
   | Service  | File                                 | Description                          |
   |----------+--------------------------------------+--------------------------------------|
   | httpd    | /etc/httpd/conf/httpd.conf           | httpd main Configuration             |
   |----------+--------------------------------------+--------------------------------------|
   | httpd    | /etc/httpd/conf.d/virtulhosts.conf   | virtual hosts entries  configuration |
   |----------+--------------------------------------+--------------------------------------|
   | AWStats  | /etc/httpd/conf.d/awstats.conf       | httpd configuration file for AWStats |
   |----------+--------------------------------------+--------------------------------------|
   | AWStats  | /etc/awstats/awstats.{{labURL}}.conf | awstats configuration files for labs |
   |----------+--------------------------------------+--------------------------------------|
   | iptables | /etc/sysconfig/iptables              | firewall rules                       |
   |----------+--------------------------------------+--------------------------------------|
   | IP       | /etc/sysconfig/network-scripts/      | network interfaces  file             |
   |----------+--------------------------------------+--------------------------------------|

* Implementation
  The implementation of the reverse proxy server and awstats is done with
  the following steps.
  
- Note :: In this implementation, variables are used in various places
          using "{}". Values of these variables are defined in
          [[common_vars.org][common_vars role]].  Use proxy_env variable if network uses
          any proxy, else comment that or remove the line.
...
** Directory structure of the scripts
   ...
   #+END_EXAMPLE
   Here =reverseproxy_server.yaml= file configures reverseproxy with
   reverseproxy_server and other roles. [[Main Configuration file][See here for other roles]]
   
   =roles/reverseproxy_server/files/httpd_awstats.conf= file is to
   configure awstats, so that user can see awstats of labs from
   specific ips. Template is created [[Allow access to awstats from management ips][here]] and task is implemented [[Apply allow access to awstats from management ips][here]]
   
   =roles/reverseproxy_server/handlers/main.yaml= file defines various
   handlers which are only executed in case a task notifies them.
   Handlers are described in detail at [[Handlers][handlers]]. The handlers are only
   called when tasks are called as part of the reverseproxy_server
   role.  Various templates and files are used during task execution
   for creating necessary configuration files.
   
   =roles/reverseproxy_server/tasks/main.yaml= file consist of various
   tasks which are needed for setting up reverseproxy server and
   awstats.
   
   =roles/reverseproxy_server/templates/awstats.reverseproxy.conf=
   file is a template to configure awstats for each lab. This template
   is created at [[Default template to configure AWStats][template]] section and the task is implemented [[Configure AWStats for each lab][here]]
   
   =roles/reverseproxy_server/templates/iptables= is a firewall rules
   template. Implementation of these firewall rules are [[Firewall rules][here]]
   
   =roles/reverseproxy_server/templates/virtualhosts.conf= is a
   virtual hosts template for adding various domains as virtual hosts
   in apache configuration file. This template is created [[Create a template for virtualhosts][here]] and
   task of this is [[Configuring virualhosts][here]]

** Tasks
*** Firewall rules
- Description of firewall rules which are common across all the nodes
  in the cluster including reverse proxy server are described in
  [[./common.org][common role]].
- Rules which are specific to reverse proxy server are described [[Allow HTTP(80) and HTTPS(443) packets][here]].

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/iptables
*filter
:INPUT ACCEPT [0:0]
:FORWARD ACCEPT [0:0]
:OUTPUT ACCEPT [0:0]
...
#Allow incoming SSH connections from configuration server IPs.  Hopefully fail2ban will take care of brute force attacks from ansible server IPs
...
####### Begin Specific to reverseproxy #######
...
# 443 OUTPUT is needed for yum
-A OUTPUT -p tcp -m tcp --dport 80 -j ACCEPT
-A OUTPUT -p tcp -m tcp --dport 443 -j ACCEPT
#Allow outgoing ping requests
-A OUTPUT -p icmp --icmp-type echo-request -j ACCEPT
#Allow incoming SSH connections from configuration server IPs.
{% for item in ansible_server_ips  %}
...
COMMIT
#+END_SRC
**** Allow HTTP(80) and HTTPS(443) packets
     The firewall rule accepts HTTP(80) and HTTPS(443) packets from the
     [[.router.org][router]].

#+BEGIN_SRC example
# Allow incoming http packets from internal network
-A INPUT -s {{local_subnet}} -p tcp -m tcp --dport 80 -j ACCEPT
#Allow incoming https packets from internal network and router
#-A INPUT -s {{local_subnet}} -p tcp -m tcp --dport 443 -j ACCEPT
#+END_SRC
*** Install required packages
    Following packages are installed for setting up reverse proxy.
    Required packages are:
    =httpd=
    =awstats=
    =mod_ssl=
- Note :: Install "mod_ssl" for HTTPS access. Enable it if it has a
          valid certificate and test the implementation using
          https://www.ssllabs.com/ssltest/index.html
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
---
...
*** Configuring firewall rules 
    The following task Copies =iptables= template created at [[Firewall
    rules section]] to reverse proxy and notifies to restart the
...
**** Enable http virtual-hosts
     To support multiple http virtual-hosts *NameVirtualHost *:80* in
     =/etc/httpd/conf/httpd.conf= is enabled with the following task.

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
- name: Configure apache to support multiple http virtualhosts
  lineinfile: line="NameVirtualHost *:80" regexp="^NameVirtualHost .*:80" dest=/etc/httpd/conf/httpd.conf

- name: Configure apache httpd.conf to listen on 80
  lineinfile: line="Listen 80" regexp="^Listen.*80" dest=/etc/httpd/conf/httpd.conf state=present

#+END_SRC
**** Enable https virtual-hosts
     To support multiple https virtual-hosts *NameVirtualHost *:443* in
     =/etc/httpd/conf/httpd.conf= is enabled with the following task.

     *NOTE* : Following playbook is having tags for https. These tags
     are added because it is causing ssl certificate issue while
     starting httpd server on reverse proxy server. Since ssl
     certificates are not available on all the clusters, httpd server
     will not start. We can run this playbook as it is without
     skipping tags on AWS. 

     When you want to run this playbook on base1 and base4 clusters,
     run the following command.
     #+BEGIN_EXAMPLE
     ansible-playbook -i hosts reverseproxy-server.yaml  --skip-tags "https"
     #+END_EXAMPLE 

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
- name: Configure apache to support multiple https virtualhosts
  lineinfile: line="NameVirtualHost *:443" regexp="^NameVirtualHost .*:443" dest=/etc/httpd/conf/httpd.conf

- name: Configure apache ssl.conf to listen on 443
  lineinfile: line="Listen *:443 http" regexp="^Listen.*" dest=/etc/httpd/conf.d/ssl.conf state=present
  tags: 
   - https

- name: Disable SSLv3 on ssl.conf
#SSLv3 will be disabled in recent version of Debian/Ubuntu, have to check for CentOS
  lineinfile: line="SSLProtocol all -SSLv2 -SSLv3" regexp="^SSLProtocol all -SSLv2" dest=/etc/httpd/conf.d/ssl.conf state=present
  tags:
   - https
- name: Enable PFS
  lineinfile: line="SSLCipherSuite AES256+EECDH:AES256+EDH" regexp="^SSLCipherSuite " dest=/etc/httpd/conf.d/ssl.conf state=present
  tags:
   - https
- name: Enforce SSL cipher order
  lineinfile: line="SSLHonorCipherOrder On" regexp="^SSLHonorCipherOrder " dest=/etc/httpd/conf.d/ssl.conf state=present
  tags:
   - https
#+END_SRC
*** Enable log forwarding to =rsyslog= node
    We configure Apache to send logs to the local =rsyslog=, which
    will then forward it to the central =rsyslog= server. The changes
    required for local =rsyslog= server on reverse proxy to further
    forward logs is configured using the [[./rsyslog-client.org][Rsyslog client]] role.
    
    We also forward access(CustomLog), error logs of lab virtual host
    to =rsyslog= node. This defined in the =virtualhosts.conf=
    template in coming section.

#+BEGIN_EXAMPLE
ErrorLog logs/{{domain.domain}}-error_log
ErrorLog "||/usr/bin/logger -t {{domain.domain}}-elog"
CustomLog logs/{{domain.domain}}-access_log combined
CustomLog "||/usr/bin/logger -t {{domain.domain}}-alog" combined
#+END_EXAMPLE

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml :eval no
- name: Enable forwarding of ErrorLog to rsyslog
  lineinfile: dest=/etc/httpd/conf/httpd.conf line='ErrorLog "||/usr/bin/logger -t http_error"' regexp="^ErrorLog.*http_.*" state=present insertafter="^ErrorLog "

- name: Enable forwarding of CustomLog to rsyslog
  lineinfile: dest=/etc/httpd/conf/httpd.conf line='CustomLog "||/usr/bin/logger -t http_access" combined' regexp="^CustomLog.*http_*" state=present insertafter="^CustomLog "

#+END_SRC

*** Create a template for virtualhosts
    Following =virtualhosts.conf= template defines http and https
    virtual-hosts.  The template uses "proxy_domains" variable which
    is defined in common_vars role. In the following template, default
    virtual host entry is added as reverse proxy server, to access the
    awstats from browser.

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/virtualhosts.conf

{% for domain in proxy_domains %}
{% if domain.domain == "reverseproxy."+prefix+"virtual-labs.ac.in" %}
<Virtualhost *:80>
...
{% elif domain.domain == "bandwidthd."+prefix+"virtual-labs.ac.in" %}
<Virtualhost *:80>
ServerAdmin engg@vlabs.ac.in
ServerName {{domain.domain}}
ServerAlias {{domain.alias}}
...
CustomLog logs/{{domain.domain}}--access_log combined
<Location />
ProxyPass http://{{domain.domain}}:8000/bandwidthd/
ProxyPassReverse  http://{{domain.domain}}:8000/bandwidthd/
</Location>
</Virtualhost>
...
CustomLog "||/usr/bin/logger -t {{domain.domain}}-alog" combined
<Location />
ProxyPass http://{{domain.domain}}/
ProxyPassReverse http://{{domain.domain}}/
</Location>
</VirtualHost>
...
{% endfor %}
#+END_SRC  
*** Configuring virualhosts
    Copy =virtualhosts.conf= template to reverse proxy server's apache
    configuration file.
 #+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
- name: Configure virtual-host for all proxy_domains
  template: src=virtualhosts.conf dest=/etc/httpd/conf.d/virtualhosts.conf
 #+END_SRC
*** AWStats configuration
**** Default template to configure AWStats
     In the following template three fields are modified in order to
     configure AWStats for each lab. ={item}= variable used as domain
     name of each lab is defined in [[./common.org][common role]] 
     =LogFile= 
     =SiteDomain=
     =Hostaliases=
    
     The actual default template to be copied to reverseproxy is 
#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/templates/awstats.reverseproxy.conf
...
#
LogFile="/var/log/httpd/{{item}}-access_log"
...
#+END_SRC
**** Configure AWStats for each lab
     1) Copy default [[AWStats configuration][awstats.reverseproxy.conf]] template to reverse proxy.
     2) creates separate awstats configuration file for each
        domain-name.
     The following task does:
     (NOTE:: This task uses awstats_domains variable and this is
     defined in common-vars role. This task)

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
#Note that extra awstats file which are not deleted when a domain is removed from awstats_domains list 
#is not an issue
- name: Create one awstats file for each proxy domain
  ...
  with_items: awstats_domains
  notify: restart httpd

- name: register configtest                                                                        
  command: apachectl configtest                                                                    
  register: op_configtest                                                                          
  notify:                                                                                          
    - restart httpd            

#+END_SRC
**** Generating and copying ssh keypair for rsyncing logs, stats to analytics server
#+BEGIN_SRC  yaml :tangle roles/reverseproxy_server/tasks/main.yaml
#Generating and copying ssh keypair for rsyncing logs, stats to analytics server
  
- name: Generate key pair under user root.
  user: name=root generate_ssh_key=yes ssh_key_file=.ssh/id_rsa ssh_key_type=rsa
    
  #Get the .pub key generated in previous step to Ansible machine, which
  # will then be used by analytics role and placed in analytics server
    
- name: Fetch the public key from RP to Ansible
  fetch: src=/root/.ssh/id_rsa.pub dest=rp_pub_key fail_on_missing=yes flat=yes validate_checksum=yes

#Set hourly cronjobs to rsync the AWstats statistics files to                                                      
#Analytics server the "name= " parameter should not be changed and                                                 
#must be unique                                                                                                    
- name: Rsync the Apache log files to analytics server                                                               
  cron:                                                                                                              
        name=rsync_apache_logs                                                                                       
        user=root                                                                                                    
        minute=01                                                                                                    
        state=present                                                                                                
        job="rsync -trp --inplace /var/log/httpd/ root@{{analytics_server_ip}}:/root/aws-http-logs/"                                                                                                             
- name: Rsync the AWstats stats file to analytics server                                                             
  cron:                                                                                                              
        name=rsync_stats                                                                                             
        user=root                                                                                                    
        minute=01                                                                                                    
        state=present                                                                                                
        job="rsync -trp --delete /var/lib/awstats/ root@{{analytics_server_ip}}:/root/aws-awstats/"  
#+END_SRC                                                                           


**** Allow access to awstats from management ips
     To access awstats from management ips , configure apache server
     configuration file =/etc/httpd/conf.d/awstats.conf= by adding
     "allow from management ips" and proper apache syntax.

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/files/httpd_awstats.conf
#
...
**** Apply allow access to awstats from management ips
     Copy [[Allow access to awstats from everywhere][httpd_awstats.conf]] configuration file to reverseproxy and
     with proper ownership, group and mode permissions for accessing
     awstats from specific ips. The following task is notified to
     restart apache web server if there are any changes in the apache
     configuration.

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml
- name: Allow access to awstats from specific ips
  copy: src=httpd_awstats.conf dest=/etc/httpd/conf.d/awstats.conf owner=root group=root mode=644
  notify:
    - restart httpd
#+END_SRC
*** Configure =logrotate=
    Logrotate is an utility which manages logs, it rotates log
    periodically depending upon the size or set time. This keeps logs
    in manageable size rather than having an single large log file
    growing without limits. It has multiple features, refer its =man=
    page for more details. Here we modify a single parameter of this
    utility in order to save =httpd= logs for a long duration.

    To configure =logrotate= for Apache, we modify file
    =/etc/logrotate.d/httpd/=, we will leave the global options in
    =/etc/logrotate.conf= unchanged. It is assumed that in the global
    option is to rotate logs weekly, this is the default when
    =logrotate= is first installed.

#+BEGIN_SRC yaml :tangle roles/reverseproxy_server/tasks/main.yaml

- name: configure logrotate to store logs for 400 weeks
  lineinfile: dest=/etc/logrotate.d/httpd regexp="^    rotate 400" line="    rotate 400" state=present insertafter=" {"

#+END_SRC

*** Install SSL certificate and configuration
    This step is preformed manually. We have used a certificate from a
    commercial third part vendor. By the end of 2015 [[https://letsencrypt.org./][Let's encrypt]]
    will be functional which offers free, automated SSL certificate
    installation.

    It is assumed that this is the first configuration of SSL on a web
    server node.

    When you purchase a SSL certificate, you get a bunch(usually 3
    files) of files which are listed below. There is no fixed
    extension, naming convention of these files, but generally these
    are named in a fashion which is easy to understand.
    
    A Server Certificate file, which generally has =.crt= extension,
    usually the naming is =<your-domain.here>.crt=.
    
    A Server Private key file, which has =.key= extension, the naming
    of this file is similar to the ceritificate file above.

    And finally a CA Certificate file, which has =.crt= extension.
    
    Place these files on the reverse proxy/web server node where you
    want to configure SSL. Next we configure Apache running on the
    node to enable SSL.

    First install =mod_ssl= package if it was not installed, the
    following command will also fetch the required dependencies.

#+BEGIN_EXAMPLE
yum install mod_ssl
#+END_EXAMPLE

    Locate the =/etc/httpd/conf.d/ssl.conf= and edit following in the
    default virtualhost.
...
SSLCertificateFile /path/to/the/server/certificate/file.crt

SSLCertificateKeyFile /path/to/server/private/key/file.pem

SSLCertificateChainFile /path/to/CA/certificate/file.crt
.
.
.
</VirtualHost>
...
#+END_EXAMPLE
    ...
    Save the file and check whether there were any syntax errors using
    =apachectl -t=, if everything is okay, then restart apache.

#+BEGIN_EXAMPLE
service httpd restart
#+END_EXAMPLE

    You can now test from a browser whether the default SSL
    virtualhost is opening over https.

    To enable SSL for other namebased virtualhost, create entires new
    =VirtualHost= entries which listen on TCP port 443.
...
- name: restart httpd
  ...
  when: op_configtest.stderr.find('Syntax OK') != -1
...
** Reverse proxy Configuration script
 ...
 4) =rsyslog-client=
 5) =nagios-client=
*** Configure Reverese proxy server 
    Configure only reverse-proxy-server without installing other clients
    ossec_client,rsyslog_client,nagios_client.
    
#+BEGIN_SRC yaml :tangle reverseproxy_server.yaml
---
- name: This file configures reverseproxy server
  ...
  vars:
   host_name: "reverseproxy.{{prefix}}vlabs.ac.in"
  roles:
    - common
    - reverseproxy_server
#+END_SRC
*** Install clients in Reverese proxy server 
    Install ossec_client,rsyslog_client,nagios_client in
    reverse-proxy-server.
    
#+BEGIN_SRC yaml :tangle install_clients_in_reverseproxy_server.yaml
---
- name: This file configures reverseproxy server
  hosts: reverseproxy_server 
  remote_user: root
