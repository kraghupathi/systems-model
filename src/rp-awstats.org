...
* Requirements
Requirements which are specific to reverseproxy defined below and all
other requirements are defined in common role's requirements
section. Please click [[./common.org][here]] for all other requirements.  The
requirements to configure reverseproxy server are as follows.
** Functional Requirements 
- Allow incoming 80 and 443 (HTTP and HTTPS) :: Reverseproxy has to
     allows incoming connections on tcp ports 80 and 443 to accept the
     web requests coming from the [[./router.org][router]] and internal network. This
     requirement will be satisfied with the [[Allow HTTP(80) and
     HTTPS(443) packets][firewall rules]]
- Allow outgoing 80 (HTTP) ::  Reverseproxy has to allow outgoing
     connections on tcp port 80 to forwards client requests which are
     coming from router to corresponding internal web server(labs) and
     sending reply to the client. This is required for yum to update
     and upgrade the OS. For this requirement look at [[Firewall
...
For more functional requirements of reverseproxy you can find [[./common.org][here]]
 
** Security Requirements 
- Accept 80 and 443 (HTTP and HTTPS) :: Reverseproxy accepts incoming
     and outgoing connections on tcp ports 80 and 443 *only* from
     internal network and from the [[./router.org][router]] .  
For more security requirements of reverseproxy you can find [[./common.org][here]]
* Design
#+CAPTION:  Reverse-Proxy Network Diagram
#+LABEL:  fig-reverseproxy-diagram
#+NAME: fig-reverseproxy-diagram
** Overall network diagram of reverseproxy
The following network diagram represents the reverseproxy server's
communication with other services and servers. In this diagram only
reverseproxy is active node. It shows the connections from/to
reverseproxy to/from other servers such as [[./router.org][router]], web servers  etc.
...
|------------+--------------------------------------+-------------------+--------------------------------------|
| Service    | File                                 | Associated with   | File Type                            |
|------------+--------------------------------------+-------------------+--------------------------------------|
