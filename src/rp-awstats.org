...
** Functional Requirements
 ...
 5) Revers proxy uses router IP as default gateway to reach the
    external world.
 6) Allow outgoing connections on tcp port 22 to analytics node, So
    that Revese proxy node will be able to transfer apache logs and
    awstats to analytics node
 7) Rsync apache-logs and awstats from the Reverse proxy node to
    Analytics node in particular time intervals(Cron jobs).
...
* Design
   ...
   Reverse proxy node transfers apache-logs and awstats to analytics
   node based on cron jobs added to crontab ( See the
   =tasks/main.yaml= file for cron jobs)
...
{% elif domain.domain == prefix+"virtual-labs.ac.in" or domain.domain == "wwww."+prefix+"virtual-labs.ac.in" %}
<Virtualhost *:80>
ServerAdmin engg@vlabs.ac.in
ServerName {{domain.domain}}
ServerAlias {{domain.alias}}
...
<Location />
ProxyPass http://{{domain.domain}}/{{domain.domain.split(".")[0]}}/
ProxyPassReverse http://{{domain.domain}}/{{domain.domain.split(".")[0]}}/
#ProxyPass http://{{domain.domain}}/
#ProxyPassReverse http://{{domain.domain}}/
</Location>
</VirtualHost>
