...
AWStats setup for analytics of labs deployed on amazon using
automated scripts.  These scripts are executed only from configuration
server.  
...
- Experiments ::
   + From command line ::
     - To test whether the reverseproxy is working or not 
        from command line , follow the steps shown below
     - added hostnames to =/etc/hosts= to my PC
     #+BEGIN_SRC 
     ...
     #+END_SRC
    - the above two commands passes through reverseproxy and gives you *index.html* file content of
      each lab
  + From Browser ::
    - Assume you have added hostnames to =/etc/hosts= in your PC
      + like below
      #+BEGIN_SRC 
      reverseproxy_ip lab1.virtual-labs.ac.in
      reverseproxy_ip lab2.virtual-labs.ac.in
    ...
    -  browse the lab URLs http://lab1.virtual-labs.ac.in  and
       http://lab2.virtual-labs.ac.in from your browser
