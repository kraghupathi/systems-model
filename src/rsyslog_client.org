#+TITLE:     Rsyslog Client - Design Model and Implementation
...
- Firewall rules applied to rsyslog client system are mentioned here. Description
  ...
  common role. Rule [[Allow outgoing connections on UDP port 514][1]] is specific to only rsyslog client and is described here.
*** Allow outgoing connections on UDP port 514
- Rsyslog client machine sends log messages to rsyslog server on UDP port 514.
#+BEGIN_EXAMPLE
...
#+BEGIN_SRC yml :tangle roles/rsyslog_client/files/rsyslog_client.conf :eval no
...
- Rsyslog service is set by following steps:
   1) Copy configuration file from the ansible machine.
...
#+BEGIN_SRC yml :tangle roles/rsyslog_client/tasks/main.yml
 - name: Copying rsyslog configuration files
   copy: src=rsyslog_client.conf dest=/etc/rsyslog.conf owner=root group=root mode=600
   notify:
     - restart rsyslog
#+END_SRC

** Set rsyslog service to auto start
#+BEGIN_SRC yml :tanlge roles/rsyslog_client/tasks/main.yml
...
#+BEGIN_SRC yaml :tangle roles/rsyslog_client/meta/main.yml
---
dependencies:
- role: common_vars
#+END_SRC
...
- Rsyslog server accepts log messages from rsyslog clients on UDP port 514. For
  machine to be configured as rsyslog client firewall rule of the machine has to
