...
- Firwall rules applied to rsyslog client system are mentioned here. Description
  of firewall rules which are common across all the systems are defined in the
  common role. Rules [[Rule for FORWARD chain][1]] and [[Allow outgoing connections on UDP port 514][2]] and are specific to only rsyslog client and are
  described here.
...
- Rsyslog client is not responsible for any packet forwarding, hence forwarding
