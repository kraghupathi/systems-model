...
** Log files for each labs on Deploy Container
   - Pattern Name :: Collection of apache access logs of each lab.
 
   - Log File Name :: A file name which collects apache logs in a
                      single file. 
  
   - Before de-commissioning the =deploy.virtual-labs.ac.in= :: The
        following tables shows list of labs running from
        =deploy.virtual-labs.ac.in= container and logs of each lab are
        stored in a file called
        =/var/log/httpd/deploy.virtual-labs.ac.in-access*= in
        =httpd.virtual-labs.ac.in= (Reverse proxy)container. These
        logs of each lab are actually old logs.
   |------+-----------------------------------+--------------------------------------|
...
** Log files for each lab hosted on IIIT Infrastructure
   The following labs hosted on IIIT infrastructure.
   |------+---------+------------------------+---------------------------------------+---------------------------------------|
...
** Log files for each lab hosted on AWS
