...
**** From Source1 
    - AWS reverse proxy server is pushing the apache logs to analytics
      server's =/root/aws/aws/httpd/= location
    - AWS reverse proxy server is pushing the awstats to analytics
      server's =/root/aws/awstats/= location.
**** From Source2
    - IIIT reverse proxy server is pushing the apache logs to analytics
      server's =/root/base/var/log/httpd/= location
    - IIIT reverse proxy server is pushing the awstats to analytics
      server's =/root/base/var/www/awstats/= location
*** Cron job to copy the logs from /root/ to /var/www/html/data/ in analytics server
    After copying the logs from sources to analytics server, analytics
    server will have a cron job to copy these logs to appropriate
    location from where analytics service uses these logs. Associated
    task for this added in tasks section.
    #+BEGIN_EXAMPLE
    rsync -arp /root/aws/* /var/www/html/data/aws/"
    rsync -arp /root/base/* /var/www/html/data/base/"
    #+END_EXAMPLE
...
- name: Copying analytics_http.conf to analytics server
  ...
  notify:
   - restart apache
#Set hourly cronjobs to rsync the AWstats statistics files and apache logs to                                                      
#Analytics server the "name= " parameter should not be changed and                                                 
#must be unique      
- name:rsync base machine's reverse proxy of aws logs to /var/www/html/data/aws/
  cron:                                                                                                              
        name= Rsync-aws-log-files
        user=root                                                                                                    
        hour=01                                                                                                      
        state=present                                                                                                
        job="rsync -arp /root/aws/* /var/www/html/data/aws/"
                                                                                                                 
- name: rsyncbase machine's reverse proxy logs to /var/www/html/data/base/
  cron:                                                                                                              
        name=Rsync-base-machine-log-files
        user=root                                                                                                    
        hour=01                                                                                                      
        state=present                                                                                                
        job="rsync -arp /root/base/* /var/www/html/data/base/"           
