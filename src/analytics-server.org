...
*** Re-configuring AWS cluster, for the requirements of analytics node
 - Analytics server configured to accept =rsync= over =TCP=, manually
   copy the public key generated on reverse proxy to analytics node
 ...
 - Router cofigured to allow incoming =rsync= connections from Source
   2(IIIT-H reverse proxy) to Analytics server
