...
* Introduction
  This document contains automation scripts for
  OS-Hardening.

  CDAC Team has done with Phase1 network_audit on our VLEAD
  infrastructure, in result, they have found some issues in
  the infrastructure and raised issues. 

  In fact, all these issues are (in a way) OS_security
  audit.  
* Requirements
  1. Harden OS by configuring certain operating system
...
** Structure of the scripts
   ...
   os-harden scripts that configures the machine. The scripts are
...
#+END_EXAMPLE
   ...
   Here =centos_harden.yml= file configures the os-harden with all
   other roles. The other roles are:
    - [[./common.org][common]]
    - centos_harden
    - rsyslog_client
    - ossec_client
...
*** Installation of Aide 
    ...
    - Impact :: Malicious misconfigurations of the file
     ...
     - Recommendations :: Install aide on centos
	  ...
	  #+END_EXAMPLE
     - Implementation :: 
#+BEGIN_SRC yml :tangle roles/os-harden/tasks/main.yml
