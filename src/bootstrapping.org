...
** ADS server
   ...
   - Generate root users ssh-keys. Setup trust based ssh from ads container root
     ...
     #+END_EXAMPLE
   - Add forced command in config server node inside =.ssh/authorized_keys= file
     of vlead user. Replace <public-key> with public key of ads machine.
     #+BEGIN_EXAMPLE
   ...
   - Make sure =/home/vlead/.ssh/authorized_keys= file has =600= permissions.
     ...
     #+END_EXAMPLE
   - Copy id_rsa.pub of ansible machine's vlead user
     i.e. =/home/vlead/.ssh/id_rsa.pub= file to ads =/root/.ssh/known_hosts=
     file.
