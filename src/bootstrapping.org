...
** Create all servers/nodes
   ...
   #+END_EXAMPLE
   - Update *cluster* variable value based on cluster name.
   - Add executable permission to =cluster.sh= script and run script
     ...
     ./cluster.sh
     #+END_EXAMPLE
     The above shell script creates all containers and they would be in running
     state.
** Router
...
** ADS server
   ...
   - Generate root users ssh-keys.  Setup trust based ssh from ads
     ...
     #+END_EXAMPLE
     - This step has to be done on config-server node. Add forced command in
       =.ssh/authorized_keys= file of vlead user. Replace <public-key> with
       public key of ads machine.
      #+BEGIN_EXAMPLE
...
- Copy id_rsa.pub of ansible machine to ads known hosts file.
- Change /etc/resolv.conf of ads to use private dns.
- Set cluster variable value in cluster.sh file for creating containers.
