	...
	cd setup-ovpl-centos/scripts/
     ...
     + OPTIONAL :: If you don't have ADS required template, you can
	  ...
	  #+END_EXAMPLE
       ...
       4. Once the network is working, Install packages, services to to
          run on start-up, create users if required.
...
*** Move /vz to a partition where there is enough space
    + By default all OpenVZ container information will go into =/vz=
      partition.
    + Some times we may not have enough space for =/vz= partition, so
      it is good to move this partition to where enough space
      available. Then symlink from the new location to =/vz=.
      #+BEGIN_EXAMPLE
...
** Create all servers/nodes
   ...
   - Add executable permission to =cluster.sh= script and run script
     ...
     ./cluster.sh
     #+END_EXAMPLE
     The above shell script all containers and they would be running
     state.
** Router 
...
** ADS server
   ...
   - Generate root users ssh-keys.  Setup trust based ssh from ads
     ...
     #+END_EXAMPLE
   - Add ssh-keys of ads machine to ansible machine.
     #+BEGIN_EXAMPLE
     ssh-copy-id vlead@10.100.1.2
     #+END_EXAMPLE
   - Clone ovpl  repository
     #+BEGIN_EXAMPLE 
   ...
   - Edit config.json as follows:
     ...
     + APP_URL :: Update app url to include FQDN
	   *Example: ads.{cluster-name}.vlabs.ac.in*
...
- Add ssh keys of ads server to ansible machine.
- add firewall rule in ansible machine to accept ssh connection from ads
  machine.
- Add ssh forced command to /home/vlead/.ssh/authorized_keys inside ansible
  machine.
