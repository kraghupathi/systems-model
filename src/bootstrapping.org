...
* Bootstrapping the cluster
  ...
  - NOTE ::  While configuring network in servers, *HWADDR* (MAC
             address of interface) value is an optional or this can be
             ignored.

** Base machine Setup 
   Need a base machine to setup cluster where bootstrapping is being
   performed. The base machine can be either physical machine or
   VM/container based on avaible resources such as CPUs, RAM, Disk
   Space etc.  
   
   The following basic hardware requirements are needed for base
   machine to setup cluster
   ...
   Have direct internet access to base machine, Router and
   Ansible/config-server IPs. So that, need not worry about proxy
   settings issues if network uses any proxy.
...
** Setup base machine with OpenVZ etc. for ADS
   ...
   - Setup base machine server with the following steps
	...
	cd setup-ovpl-centos/scripts/
       #+END_EXAMPLE
     + Comment installation of mongodb in =centos_prepare_ovpl.sh= file - (line
       60-66 ), as we do not need it on host machine.
...
** Create all servers/nodes
   ...
   then 
   vzctl set $i --hostname config-server.${cluster}.vlabs.ac.in --save
   vzctl set $i --netif_add eth0,,,,br0 --save
   vzctl set $i --netif_add eth1,,,,br1 --save
   vzctl start $i
   vzctl set $i --onboot yes --save
   ...
   #+END_EXAMPLE
   - Add executable permission to =cluster.sh= script and run script
     as:
     #+BEGIN_EXAMPLE
     chmod +x cluster.sh
     ./cluster.sh
...
** Configure Router 
   ...
   - Enable ssh access from config-server node
     ...
     #+END_EXAMPLE
# COMMENT:      - NOTE :: Reason to run router.yaml play book here is: We added
...
**** Purchased ssl certificate 
     - If already have purchased ssl certificates, configure
...
**** Self signed ssl certificates
     Use self signed certificates if no purchased ssl certificates are
     available.
     ...
     - Generate self signed certificate as follows:
       ...
       #+END_EXAMPLE
     - Open the file =/etc/httpd/conf.d/ssl.conf=. Look for
       =SSLCertificateFile= and =SSLCertificateKeyFile=
       directives/fields and provide full path of certificates
       created above to those directives.
    - Restart httpd service if apache syntax is correct
      #+BEGIN_EXAMPLE
      apachectl -S         # Apache configuration syntax check
      service httpd restart
...
** Run site.yaml from config-server
    - Set nameserver for ads server to use private dns.
      ...
      #+END_EXAMPLE
    - Check management_ips in common_vars role and update them
      appropriately.
    ...
    - Uncomment =rsnapshot_client= in all the roles here
...
** Host labs using ads service
   - Deploy a few labs using ads service to test the whole setup. Open
     http://ads.{cluster}.vlabs.ac.in:8080 and provide required
     fields.
     - How to host a lab :: 
       ...
       4. After logged in you will see the web page showing with
          three fileds:
	  + *Lab ID* - In this field provide lab_d(e.g cse03-iiith,
            ccnsb03-iitk etc.)

	  + *Repo URL* - Provide lab GitHub repository URL. 
			  Example:
	  ...
	  + *Tag* - This is optional field, we can ignore.
       ...
       6. If everything goes well, you will get an IP of lab where
          lab has been hosted.
	  - Note :: You can see the logs of lab hosting process in ads
                    server using
		    #+BEGIN_EXAMPLE
		    tail -f /root/logs/ovpl.log
		    #+END_EXAMPLE
   - After deploying the lab, update the entries in common_vars of
     following variables appropriately.
     #+BEGIN_EXAMPLE
     proxy_domains:
   ...
   - Then run ansible-playbooks of three roles as follows:
     ...
     ansible-playbook -i hosts private-dns.yaml
     #+END_EXAMPLE
   - Check whether the lab page has been serving or not using provided
     lab url using any browser.
** On Base machine
   ...
   - Stop all containers and reboot base machine.  Verify that things
     automatically work after reboot. If nameserver is not set to
     10.100.1.5 in all the containers, we have to do the *Reboot
     related steps*.
...
* ADS hooks related things to do in boot strapping
- Copy id_rsa.pub of ansible machine to ads known hosts file. *Done*
- Set cluster variable value in cluster.sh file for creating containers. *Done*
- Change /etc/resolv.conf of ads to use private dns. *Done*
- Update makefile to replace cluster variable in config_server.yml with cluster
  value. *Done*
- Set environment path for root user and vlead user. This is achieved by
  appending path in /etc/environment. And then writing source /etc/environment
  in .bashrc file. *Done*
- In hooks.sh change file permissions of vars.yaml to be writable by vlead user,
  run the service and then roll back file permission. *Done*
- Update latest code from base4 to model. *Done*
- Comment requiretty in /etc/sudoers file. *Done*
- Add vlead user to sudoers file and set permissions to execute all commands
  with NOPASSWD. *Done*
- Run ansible playbook as sudo user. *Done*
- SSH forced command is to run 'hooks.sh'. *DONE*
- Make service program executable. *Done*
