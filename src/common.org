...
assumes the first line in =/etc/hosts= is the machines own FQDN to IP mapping
...
machine. For each server in the cluster value of these variables will be
...
*** Objective
To check bind-utilities are installed on the server.
*** Apparatus
...
**** SSH to the server 
SSH to the server from any other machine by intentionally giving wrong
...
machine ip from where multiple failed login attempts are made.
#+BEGIN_EXAMPLE
iptables --list
#+END_EXAMPLE
