...
# comment Should be 'This document describes'
...
# comment It is not clear what are  verifications, it can probably be removed
  possible. Test Cases are also described to test the setup.

  Our cluster consists of many nodes. Every node has a specific
# comment 'our cluster' is not the right word. replace 'our' with something else such as Base1/AWS
...
# comment There should be a comma after 'one cluster'
...
# comment unable to view the above diagram
...
# comment spelling mistake - common, not 'cmmon'
...
# comment should be 'applied to' rather than 'applied in'
