...
common roles. A =role= in Ansible terminology is a specific set of configuration
and =common roles= is a role which is applied on all the deployed servers.
...
- Firewall rules which are applied across all the servers are described here.
  Rules which are specific to the individual server are described in the
  individual server documentation itself.
...
- This rule limit new connections apart from UDP connections. Limit is set to a
...
- Allow server to accept incoming ping requests from anywhere.
#+BEGIN_EXAMPLE
...
- Allow server to continue already related and established connections. Previous
  connection would have passed through the firewall and if they got established,
...
- Allow server to accept incoming ssh TCP port 22 connections *only* from the
  ansible, nagios and management ips. For loop is used to enable rule for
...
- Allow input connections from cacti server.
#+BEGIN_EXAMPLE
...
- Log the packets which are to be rejected with a limited rate.
#+BEGIN_EXAMPLE
...
- Allow server to continue already related and established connections. Previous
  connection must have passed through the firewall and if they got established,
  connection is allowed to continue.
#+BEGIN_EXAMPLE
#Allow to continue already related and established connections
...
- Allow system to send log messages to rsyslog server.
#+BEGIN_EXAMPLE
#Allow to make dns queries
...
- Allow logwatch service to send mail alerts.
#+BEGIN_EXAMPLE
...
- Allow yum service to update packages.
#+BEGIN_EXAMPLE
...
- Allow system to send packets to OSSEC server.
#+BEGIN_EXAMPLE
...
|------+-------------------------+------------+----------+------------------------------------------------------|
|   1. | April 9, 2015, Thursday | 09:20 a.m. |          | - Confirm tangle output is same as aws implmentation |
|      |                         |            |          | - Start modeling common role                         |
