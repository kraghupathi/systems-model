...
*** Setup /etc/hosts file
   ...
   Following variables are set by the ansible tool as it runs on the
   client node. For each server in the cluster value of these
   variables will be different and set according to the server role.
