...
**** Bogus Messages are accepted with the server
...
#+BEGIN_SRC yml :tangle  roles/common/tasks/main.yml
- name: Bogus Messages are accepted with the server
  lineinfile: 
    dest:  /etc/sysctl.conf
    line: '{{ item }}'
  with_items:
...
- name: Modify active kernel parameters to match
  command: "{{ item }}"
  with_items:
    - /sbin/sysctl -w net.ipv4.icmp_ignore_bogus_error_responses=1
    - /sbin/sysctl -w net.ipv4.route.flush=1
...
- name: Configure /etc/hosts.allow
  lineinfile: 
     ...
     regexp: "^ALL"
     line: "ALL: 10.100.0.0/255.255.252.0, 10.0.0.0/255.255.252.0, .vlabs.ac.in"
#+END_SRC
...
- name: set the active kernel parameter
