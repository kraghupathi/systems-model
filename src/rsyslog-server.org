...
* Requirements
   ...
   specified in the [[./common.org][common role]] are mentioned here.
...
** Security Requirements
   All common security requirements as described in [[file:common.org::*Security%20Requirements][common role]] are
...
#+END_EXAMPLE
   ...
   The handlers are only called when tasks described at Building
...
*** Firewall Rules
    ...
    installed by ansible task Set iptables firewall rules using template.
...
#+END_SRC
    ...
    This configuration file is installed on rsyslog server using Set
    Rsyslog service ansible task described later.

** Tasks
*** Install rsyslog package
    Rsyslog package is installed on the node using Yum ansible module
...
*** Configure iptables firewall rules using template
    ...
    1. Copy iptables jinja2 template created in section [[Firewall
       Rules][Firewall rules]] from the ansible
       node to the rsyslog node.
...
** Building rsyslog server hosts file with appropriate roles
   ...
   role.
*** Configure only rsyslog-server
    - Configure only rsyslog-Server without installing other clients
...
*** Install clients in rsyslog-server
    Install ossec_client,rsyslog_client,nagios_client in
    rsyslog-server.
    
#+BEGIN_SRC yaml -n :tangle install_clients_in_rsyslog_server.yaml
---
- name: Configure rsyslog server
  hosts: rsyslog_server
  remote_user: root
