...
---
- name: CentOS -Install rsyslog package
  yum: name=rsyslog state=present

- name: Debian - Install rsyslog package
  apt: name=rsyslog state=present
  when: ansible_disribution == "Debian" or ansible_distribution == "Ubuntu"
...
** Rsyslog configuration script
   Rsyslog server is configured using =rsyslog_client= role.
