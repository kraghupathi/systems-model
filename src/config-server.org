...
public key of configuration server will be placed into all aother
...
|   |   ...
#+END_EXAMPLE
Here =config_server.yml= file configures config_server host with
config_server and all other roles.
...
#+END_SRC
** Handlers
  Restarts the iptables service if anything is modified in
  configuration file of iptables service.  These handlers are called
  only when tasks are notified to start any service.
