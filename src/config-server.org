...
public key of configuration server will be placed in all aother
...
** Functional Requirements
  ...
  3. Only authorized VLEAD members can log-in as 'root' user from
     IIIT IP range or management ip machines using key based authentication only.
...
The following network diagram shows us how the configuration server is
connecting to other nodes such as reverseproxy, DNS, router etc.
...
SSH access to this node allowed only on key-based authentication,
...
which are only only executed in case a task notifies them. Handlers
...
#+END_SRC
** Handlers
  Restarts the iptables service if configuration file of iptables is
  modified. These handlers are called only when tasks are notified to
  start service.
...
**** SSH to the config_server from management ips/ IIIT ips
    ...
    of this server is generated during creation of the VM. So
