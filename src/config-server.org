...
* Introduction
  ...
  # We need not mention here that it will be called as node
  # because in index.org I have alread mentioned that all these are
  # nodes.
  ...
  # Since you have mentioned that it plays a vital role please
  # justify this statement by mentioning how it is vital component in
  # the cluster.
  
  Configiuration server configures all other nodes in the cluster and
  configures itself as well, through multiple roles and tasks.  A role
  is a specific set of configurations.
  # 1.) Describe a little more about the config server. That is,
  # mention that it contains all the ansible-scripts to bring up the
  # other nodes. The purpose of this server is to avoid mannual
  # configuration of nodes by team members. The mechanism ansible
  # provides to configure other nodes is via ssh. etc, etc. Also
  # describe what you mean by configures itself. This will help the
  # reader understand the requirements better. 2.) Also we need not
  # mention about roles here specifically. I have written in the
  # index.org prerequisites that the reader is expected to know
  # ansible basics. In case you have written anywhere else, please
...
* Requirements
  Requirements which are specific to configuration server are
  described below and all other common requirements are described in
  [[./common.org][common role]]
# A better way of putting the sentence above would be: The
# requirements of the config-server which differ from the generic
...
# It should be "the" configuration server accepts...

* Design
  *The design of the configuration server is as follows*:
  # Why is the above line in bold ? I feel this line is not
  # required. Also to maintain consistency among all the models I
  # would suggest you to not to have this line.
  The following network diagram represents how the configuration
  server is connecting to other nodes such as reverseproxy, DNS,
  router etc.

  #+CAPTION:  Configuration Server Design Diagram
  #+LABEL:  fig-config-diagram
  [[./diagrams/config-server-network-diagram.png]]
  # resize the figure and remove the extra white spaces
  ...
  # change ip -> IP in the above para
  The design of the firewall rules ensures that this server is
  accessible only via port 22.  
  # the above line can be mentioned in the implementation section,
  # where firewall rules are explained.
  ...
  # break the first sentence into 2 sentences. Full stop after
  # key-based authentication.  The entire paragraph can be reframed.
  
  The Configuration serevr can be reached via ICMP (ping) requests.
  # please do a thorough spell check. Thereare still some spelling
  # mistakes.
  ...
  configure all other nodes, instances/containers and as well as itself
  by running the ansible scripts. These ansible scripts are implemented
  at the [[Implementation][implementation]] section.
# these ansible scripts are described in the implementation section.

** Configuration files Summary
# change the heading to "Important configuration and data files and folders"
...
# the ip can be removed as it is implicit and is not required to be
# mentioned.  I guess you should mention the ssh key location too
# here. Because that is the main thing in ansible server. Without the
# keys it will not function. 
# Also change "description" to "purpose" 
...
|   |   ...
#+END_EXAMPLE
# remove the common in the above tree.
   ...
   them. Handlers are described in details at [[Handlers][handlers]].  The hanlders
   are caled only when tasks are called as part of the config_server
   role. Various templates and files are being used during task
   execution for creating necessary configuration files.

   # put this paragraph below the tasks paragraph.
   ...
   created at [[Firewall rules]] section and you can see the decription
   each firewall.
