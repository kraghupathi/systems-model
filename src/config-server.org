...
Configuration server's authorized keys with ssh forced command enabled to make 
sure that only register/deregister service is called by ADS.
...
yml files of various server. The structure of yml files is as follows:

#+BEGIN_EXAMPLE
- { hostname: lab1, ip: 10.100.1.10 }
- { hostname: lab2, ip: 10.100.1.12 }
#+END_EXAMPLE

*** Service Program
Following program updates the entries in the data file with the values passed by
ADS service. This service is invoked by the ADS server. Service registers or
de-registers the lab.

**** Register Service
Flow of the function is as follows:

- 1 :: Read data from input file
- 2 :: Save existing entries in a variable
- 3 :: Add new entry to the variable if not already present 
- 4 :: Write the new entry along with all the old entries in a file
- 5 :: Call update functions to update reverseproxy and dns entries
...
def register(hostname, ip):
    ...
    setOfEntries = set(existingEntries)
    setOfEntries.add(hostnameIP)

    writeLabFile = open("lab.txt", "w")
    for item in setOfEntries:
...
- 1 :: Read data from input file
- 2 :: Save existing entries in a variable
- 3 :: Delete new entry from the variable if present 
- 4 :: Write the new entry along with all the old entries in a file
- 5 :: Call update functions to update reverseproxy and dns entries
...
def deregister(hostname, ip):
    ...
    try:
        indexDelete = existingEntries.index(hostnameIP)
        del existingEntries[indexDelete]
    except:
        pass
...
- 1 :: Call register service if argument passed is register
- 2 :: Call deregister service if argument passed is deregister
- 3 :: Read path values of files from "path.txt" file

#+BEGIN_SRC txt :tangle path.txt :eval no
dns main.yml
nagios main.yml
reverseproxy main.yml
rsnapshot main.yml
#+END_SRC

- 4 :: Invoke functions to make entries in various servers
...
def main(argv):
    ...
    for line in pathFile.readlines():
        ...
        if splitLine[0] == "reverseproxy":
               revpath = splitLine[1]
        elif splitLine[0] == "dns":
               dnspath = splitLine[1]
        elif splitLine[0] == "nagios":
               nagiospath = splitLine[1]
        elif splitLine[0] == "rsnapshot":
               rsnapshotpath = splitLine[1]

    reverseproxy("lab.txt", revpath)
    dns("lab.txt", dnspath)
    rsnapshot("lab.txt", rsnapshotpath)
    nagios("lab.txt", nagiospath)
...
- 2 :: Find start of the DNS entry section
...
def dns(inputFile, outputFile):
    ...
    newYmlData = ymlData[0:dnsStartIndex + 1] + dnsNewEntries + delimeter + ymlData[dnsEndIndex+1:]
...
- 1 :: Read data from the yml file
- 2 :: Find start and end of the "Domain" and "AWSTATS" entry section
- 3 :: Read values from input data file
- 4 :: Save all the new entries in a variable
- 5 :: Combine existing data and new entries
- 6 :: Write all the entries in the yml file
...
def reverseproxy(inputFile, outputFile):
    ...
    newYmlData = ymlData[0:domainStartIndex + 1] + domainNewEntries + ymlData[domainEndIndex:awstatsStartIndex+1] + awstatsNewEntries + ymlData[awstatsEndIndex:]
