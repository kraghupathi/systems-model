...
#+BEGIN_SRC python :tangle format.py :eval no
    ...
    newYmlData = ymlData[0:domainStartIndex + 1] + domainTotalEntries + delimeter + awstatsTotalEntries + ymlData[awstatsEndIndex:]
#+END_SRC
...
#+BEGIN_SRC python :tangle format.py :eval no
    return
#+END_SRC
*** De-registering DNS Entries
DNS entries of the labs are removed from the configuration files of the DNS
server. FQDN and ipaddresses are read from the data file.
**** Function definition
#+BEGIN_SRC 
def dns_dereg(inputFile, outputFile):
#+END_SRC
**** Open Input File and Output File
Open input file - "lab.txt" to read the content. Also open output file -
"private_dns.yaml" to write data.
...
#+BEGIN_SRC python :tangle format.py :eval no
    ...
    for index in range(dnsStartIndex + 2, dnsEndIndex):
        dnsExistingEntries.append(ymlData[index])
#+END_SRC
**** Remove entries from the variable
New entries are appended in the same variable with proper format.
#+BEGIN_SRC python :tangle format.py :eval no
    for line in labFile.readlines():
        splitLine = line.split()
	hostnameAndIP = " - { hostname: %s, ip: %s }\n" %(splitLine[0], splitLine[1])
	try:
            print dnsExistingEntries
            print dnsExistingEntries[6]
            print hostnameAndIP
            indexDelete = dnsExistingEntries.index(hostnameAndIP)
            del dnsExistingEntries[indexDelete]
            print "Entry deleted"
	except:
...
#+BEGIN_SRC python :tangle format.py :eval no	
    delimeter = list("#DNS_END\n")
    newYmlData = ymlData[0:dnsStartIndex + 2] + dnsExistingEntries + delimeter
#+END_SRC
**** Close file pointers
#+BEGIN_SRC python :tangle format.py :eval no
    labFile.close()
...
#+BEGIN_SRC python :tangle format.py :eval no
    return
#+END_SRC
*** De-registering Reverseproxy Entries
Reverseproxy entries of the labs are removed from the configuration files which
labs are to be de registered. Once the entries are removed lab wont be
accessible from external world.
**** Function definition
#+BEGIN_SRC python :tangle format.py :eval no
def reverseproxy_dereg(inputFile, outputFile):
#+END_SRC
**** Open Input File and Output File
Open input file - "lab.txt" to read the content. Also open output file -
"private_dns.yaml" to write data.
...
#+BEGIN_SRC python :tangle format.py :eval no
	...
	awstats = '  - %s\n' %(fqdn)
        try:
            indexDelete = domainExistingEntries.index(hostnameAndIP)
            del domainExistingEntries[indexDelete]
            indexDelete = awstatsExistingEntries.index(awstats)
            del awstatsExistingEntries[indexDelete]
	except:
	    pass
#+END_SRC
**** Complete file content
#+BEGIN_SRC python :tangle format.py :eval no	
    delimeter = list("#DOMAIN_END\n\n#AWSTATS_START\nawstats_domains:\n")
    newYmlData = ymlData[0:domainStartIndex + 2] + domainExistingEntries + delimeter + awstatsExistingEntries + ymlData[awstatsEndIndex:]
#+END_SRC
**** Close file pointers
#+BEGIN_SRC python :tangle format.py :eval no
    labFile.close()
...
*** Service Program
This service is invoked by the ADS server. Service registers or de-registers the
lab.
...
def register(hostname, ip):
    ...
    labFile = open("reg-lab.txt", "a")    
    labFile.write("%s %s\n" %(hostname, ip))
    labFile.close()
    reverseproxy_reg("reg-lab.txt", "main.yml")
    dns_reg("reg-lab.txt", "main.yml")
    return

def deregister(hostname, ip):

    labFile = open("dereg-lab.txt", "a")
    labFile.write("%s %s\n" %(hostname, ip))
    labFile.close()
    reverseproxy_dereg("dereg-lab.txt", "main.yml")
    dns_dereg("dereg-lab.txt", "main.yml") 
    return
...
** TODO Write a tasks in playbook to place the code in proper directory, set PATH environment variable.
** TODO Separate out the lab entries from the node models. Because in future ads
...
** DONE Write python program to convert data saved in lab.txt to proper yml file for DNS, reverseproxy
** DONE Add requirement section of ADS hooks in config-server.org model file
** DONE Add description for implementation section in ads hooks
** DONE Add description for design section in ads hooks
** DONE Modify Make file to tangle out the code
** DONE Write python program to deregister the lab
