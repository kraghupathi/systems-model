...
called.
#+BEGIN_SRC yaml :tangle roles/config_servers/templates/hooks.sh
#!/bin/sh
...
#+BEGIN_SRC yml :tangle roles/config_server/tasks/main.yml
- name: Copy hooks.sh file to config server
  template: src=hooks.sh dest=/etc/resolv.conf owner=root group=root mode=755
#+END_SRC
