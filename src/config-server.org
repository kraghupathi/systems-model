...
**** Comment requiretty from /etc/sudoers file
Ads invokes hooks service via ssh and in order to be able to execute command as
sudo in config server we have to disable 'requiretty'. So the ssh connection
will not require tty.
#+BEGIN_SRC yaml :tangle roles/config_server/tasks/main.yml
- name: comment requiretty
  replace: dest=/etc/sudoers regexp='^Defaults .* requiretty$' replace='#Defaults    requiretty'
#+END_SRC
**** Add vlead user to sudoers group
Hooks service execute playbook as vlead user, and in order to change permissions
of file at run time it need sudo privileges.
#+BEGIN_SRC yaml :tangle roles/config_server/tasks/main.yml
- name: add vlead user to sudoers group
  lineinfile: "dest=/etc/sudoers regexp='^vlead' insertafter='^root' line='vlead   ALL=(ALL)    NOPASSWD: ALL'"
#+END_SRC
**** Define environment variable in /etc/environment
#+BEGIN_SRC yaml :tangle roles/config_server/tasks/main.yml
#- name: add vlead user to sudoers group
#  lineinfile: "dest=/home/vlead/.bashrc regexp='^vlead' insertafter='^root' line='export PATH=$PATH:'"
#+END_SRC
**** Handlers
...
baseDir='/home/vlead/systems-model/build/'
cluster=$(grep "^cluster=" ../../makefile | awk -F '=' '{print $2}')
variableFile='/roles/common_vars/main.yml'

sudo chmod 777 $baseDir$cluster$variableFile

hosting_service $arg2 $arg3 $arg4

sudo chmod 644 $baseDir$cluster$variableFile
...
def main(argv):
    ...
    if (str(argv[0]) == "register"):
        ...
        if (returnCode != code['regError']):
            ...
            update_reg(labData)
            f = open("/dev/null","w")
            #call(["ansible", "all", "-i", "localhost,", "-c", "local", "-m", "shell", "-a", "echo hello world"])
