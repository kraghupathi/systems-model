...
**** Define ssh forced command
Add ssh forced command to .ssh/authorized_keys file of vlead user. ADS server
runs hooks service from ads server.
#+BEGIN_SRC yaml :tangle roles/config_server/tasks/mail.yml
- name: add ssh forced command in /home/vlead/.ssh/authorized_keys
  lineinfile: dest=/home/vlead/.ssh/authorized_keys regexp=^command line=command="hooks.sh",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty ssh-rsa <public_key>
#+END_SRC
