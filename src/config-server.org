...
#+BEGIN_EXAMPLE
ssh ansible@ansible.vlabs.ac.in service <register/deregister> <labid> <ipaddress>
#+END_EXAMPLE
...
1. Call register function if argument passed is register
2. Call deregister function if argument passed is deregister
3. Call update function to update yml files

#+BEGIN_SRC python :tangle hosting.py :eval no
#!/usr/bin/env python
...
1. Call findEntry function to check if entry already exists
2. Write entry to data file if entry does not exist
3. Throw error if entry already exist in data file
...
def register(hostname, ip):
    ...
    if (index >= 0):
        print "Error: Lab is already registered\n"

    else:
        print "Registering the lab\n"
        hostnameIP = "%s %s\n" %(hostname, ip)
        entries.append(hostnameIP)
	writeLabFile = open("lab.txt", "w")
	for item in entries:
...
1. Call findEntry function to check if entry already exists
2. Delete entry from data file if exist
3. Throw error if entry does not exist in data file
...
def deregister(hostname, ip):
	...
	print "Deregistering the lab\n"
        ...
        print "Error: Lab is not registered\n"
...
1. Read data from data file into a list
2. Search for entry in the data file
3. Return index of entry if entry exist in the file
4. Return -1 if entry does not exist

#+BEGIN_SRC python :tangle hosting.py :eval no
def findEntry(hostname, ip):
    
    readLabFile = open("lab.txt", "r")
    hostnameIP = "%s %s\n" %(hostname, ip)

    labData = readLabFile.readlines()
    readLabFile.close()

    indexCount = len(labData)
    existingEntries = []

    for index in range(0, indexCount):
        existingEntries.append(labData[index])

    try:
        index = existingEntries.index(hostnameIP)
        return index, existingEntries 
    except:
        return -1, existingEntries
...
#+BEGIN_SRC python :tangle update.py :eval no
delimiters = {}
...
def update(inputFile):
	...
	entries['dns'].append(dns(line))
	entries['reverseproxy'].append(reverseproxy(line))
	entries['nagios'].append(nagios(line))
	entries['awstats'].append(awstats(line))
...
**** Reverseproxy
Create data in the proper format to be updated in the reverseproxy variable section.
#+BEGIN_SRC python :tangle update.py :eval no
def reverseproxy(line):
    splitLine = line.split()
    fqdn = splitLine[0] + ".virtual-labs.ac.in"
