...
the configuration server.  This server configures all other servers in
the cluster and configures it self as well, through multiple roles and
tasks.  A role is a specific set of configurations.

In this documentation, we used to call config-server as ansible server
# comment replace 'we used to call' with 'we call'
some times. 

This configuration server(config-server) will be able to ssh to other
servers and lab instances/containers using *key based authentication
only* since public key of configuration server is copied into all the
servers and instances/containers. Password based authentication is not
allowed for root user.

Only from management ip machines, we will be able ssh to root account
...
# comment should be 'are described below'
and all other common requirements are described in [[./common.org][common role]]
...
The following network diagram tells us how the configuration server is
...
This node is one of the many other nodes of the cluster.  Only from
IIIT ip range and management ip machines are allowed to ssh to root
account of the configuration server. SSH access to this node allowed
only on key-based authentication, password based authentication is not
allowed to root user. The design of the firewall rules ensures that
this server is accessible only via port 22. This server can be
reached via ICMP (ping) requests.

SSH configuration, fail2ban and locking root user password are taken
care by common role. For more tasks which are done by common role in
this node, please go through [[./common.org][common role]]. 

This node's public key will be placed inside every
instance/container. So that configuration server will be able to
...
by which role configurations are applied. Other configuration files
are updated in common role.
|-----------+-----------------------------------+---------------------------|
...
- Note1 :: In this implementation, variables are used in different
          ...
          definition, please go through the common_vars role.
...
**** SSH to config_server from workstation with unknown ip
    Suppose a workstation with unknown ip and it has KeyPair, then
    ssh to configuration server from that workstation with unknown
