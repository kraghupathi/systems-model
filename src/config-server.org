...
This configuration server plays a vital role in this cluster to setup
# comment replace 'this' with 'the'. 'The configuration server...in the cluster'
the complete infrastructure.   

This server configures all other servers in the cluster and configures
# comment replace 'this' with 'the'. 'The configuration server'
itself as well, through multiple roles and tasks.  A role is a
specific set of configurations.

This configuration server will be able to ssh to other servers and lab
# comment replace 'this' with 'the'. 'The configuration server'
...
* Requirements
Requirements which are specific to configuration server are described
below and all other common requirements are described in [[./common.org][common role]]
** Functional Requirements
  ...
  3. Only authorized VLEAD members can log-in as 'root' user from IIIT
     IP range or management ip machines using key based authentication
     only.
** Security Requirements
   Configuration server accepts incoming connections on port 22 *only*
   from IIIT IP range or management ip machines.

* Design
*The design of this configuration server is as follows*:
# comment replace this with the 'the configuration server' 

The following network diagram represents how the configuration server
is connecting to other nodes such as reverseproxy, DNS, router etc.
#+CAPTION:  Configuration Server Design Diagram
#+LABEL:  fig-config-diagram
[[./diagrams/config-server-network-diagram.png]]

This node is one of the many other nodes in the cluster.  Only from
# remove from in above sentence
IIIT ip range and management ip machines are allowed to ssh to root
account of the configuration server. 

SSH access to this node is allowed only via key-based authentication,
password based authentication is not allowed to root user. The design
of the firewall rules ensures that this server is accessible only via
port 22.

SSH configuration, fail2ban and locking root user password are taken
care by common role. For more tasks which are done by common role,
please go through [[./common.org][common role]].

This node can be reached via ICMP (ping) requests.
# comment replace 'this' with something else

This node's public key will be placed in every instance/container. So
# comment replace 'this' with something else
that configuration server will be able to configure all other nodes,
instances/containers and as well as itself by running the ansible
scripts. These ansible scripts are implemented at [[Implementation]]
section.
...
|   |   ...
#+END_EXAMPLE
Here =config_server.yml= file is a main playbook to configure
configuration server host with config_server and all other roles.

=roles/config_server/handlers/main.yml= file defines various handlers
<<<<<<< HEAD
# commment It is not clear what is meaant by <<<<< HEAD
which are only executed in case a task notifies them. Handlers
are described in details at [[Handlers][handlers]].  The hanlders are caled only
when tasks are called as part of the config_server role. Various
templates and files are being used during task execution for creating
necessary configuration files.
=======
which are only executed in case a task notifies them. Handlers are
described in details at [[Handlers][handlers]].  The hanlders are caled only when
tasks are called as part of the config_server role. Various templates
and files are being used during task execution for creating necessary
configuration files.
>>>>>>> fd313fa0fba89ba5decb920f54fb1c5f47bb86f7
# comment It is not clear what above statement is,  starting with >>>>>
...
=roles/confi_server/templates/iptables= template file consist of
various firewall rules in order to configure this node securly by
...
created at [[Firewall rules]] section and you can see the decription each
firewall.
...
{% for item in ansible_server_ips_ips  %}
-A INPUT -m state --state NEW -s {{item}} -p tcp -m tcp --dport 22 -j ACCEPT
{% endfor %}
...
The following firewall rule accepts incoming connections on tcp port
22 only from ansible server ips
#+BEGIN_SRC example
{% for item in ansible_server_ips_ips  %}
-A INPUT -m state --state NEW -s {{item}} -p tcp -m tcp --dport 22 -j ACCEPT
{% endfor %}
...
itself and to run the ansible scripts to configure the other nodes in
the cluster.
...
#+END_SRC
*** Change hostname 
  The following task changes the hostname of the container/VM
  #+BEGIN_SRC yaml :tangle roles/config_server/tasks/main.yml
  ---
- name: changing the hostname 
  hostname: config-server.vlabs.ac.in
  #+END_SRC
