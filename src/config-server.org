...
* Test Cases
** Test case ID :: SSH access restrictions 
*** Objective
    Test to check if SSH is allowed only from IIIT network for the user
    root and denied from other ips
*** Apparatus
    1. A machine with IIIT IP 
    2. A machine with unknown ip 
    3. Configuration server 
       This configuration server created on aws using AWS console.
*** Procedure
    1. Assume you are inside IIIT network.
    2. Place/copy the keyPair which is created by amazon during
       creation of the VM into ssh-tunnel container.
    3. Give 400 permission to that KeyPair and
    4. ssh to config_server
*** Experiments
**** SSH to the config_server from management ips
    ...
    the private KeyPair is stored in "ssh-tunnel" container on
    ...
    #+BEGIN_SRC example
    ssh -i <KeyPair-name> root@config_server-ip
    #+END_SRC
***** Expected result
   #+BEGIN_EXAMPLE
...
**** SSH to config_server from unknown ip
    Suppose a machine with unknown ip and it has private KeyPair, then
    ssh to configuration server from that machine with unknown
    ip(other than IIIT network ip) using bellow command

    #+BEGIN_SRC example 
    unknown-machine$ ssh -i <keypair-name> root@config_server-ip   
    #+END_SRC
***** Result
