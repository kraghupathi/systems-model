...
Set environment variable "PATH" so the service can be invoked from anywhere
independent of the same working directory where file is present.

#+BEGIN_SRC yaml :tangle roles/config_server/tasks/main.yml
---
- name: set environment variable
  lineinfile: dest=/etc/environment regexp=^PATH= line=PATH=/usr/lib64/qt-3.3/bin:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin
#+END_SRC
...
#+BEGIN_EXAMPLE
ssh ansible@ansible.vlabs.ac.in service <register/deregister> <hostname> <ipaddress>
#+END_EXAMPLE
...
**** Main Function
The main function provides help to the user about the functionality of the
service.  Its main job is to register the lab if the argument passed is
'register' and deregister the lab if the argument passed is 'deregister'. It
updates the yml configuration file. Error code is thrown if the ads service
tries to register existing lab or tries to deregister non-existent lab.

#+BEGIN_SRC python :tangle hosting_service.py :eval no
#!/usr/bin/env python

import sys
from config import *
from modularised_program import *
...
def main(argv):
    ...
    if (returnCode != code['regError'] and returnCode != code['deregError']):
	update()
    else:
	sys.exit(123)
...
**** Register
Register function finds the entry of the lab to be registered in the data
file. If the entry is not present then it appends the new entry in the data
file. But if the entry is already present then it throws an error.
...
**** Deregister
Deregister function finds the entry of the lab to be deregistered in the data
file. If the entry is present then it deletes the entry from the data file. But
if the entry is not present then it throws an error.
...
**** Findentry
Findentry function finds the entry of a lab in the data file.
If the entry exist function returns the index of the entry, if the entry is not
found it returns -1. Existent entries are also returned.
...
5. *serviceHelp* - Docstring of the service to provide help to users

#+BEGIN_SRC python :tangle config.py :eval no
delimiters = {}
delimiters['dns'] = ["#DNS_START\n", "#DNS_END\n"]
delimiters['reverseproxy'] = ["#DOMAIN_START\n", "#DOMAIN_END\n"]
delimiters['awstats'] = ["#AWSTATS_START\n", "#AWSTATS_END\n"]
delimiters['nagios'] = ["#Nagios_START\n", "#Nagios_END\n"]

fileLocation = {}
fileLocation['dns'] = 'main.yml'
fileLocation['nagios'] = 'main.yml'
fileLocation['reverseproxy'] = 'main.yml'
fileLocation['awstats'] = 'main.yml'

labPath = "/root/lab.txt"
...
**** Update
Update function gets the proper format of the data for various servers DNS,
reverseproxy, awstats and nagios. Update function calls the server's function to
represent the data in the respective format. Then call *append_data* function to
append data in the yml files.
...
def update():
    ...
    entries = {}
    entries['dns'] = []
    entries['reverseproxy'] = []
    entries['nagios'] = []
    entries['awstats'] = []

    labFile = open(labPath, "r")
    
    for line in labFile.readlines():
	entries['dns'].append(dns_format(line))
	entries['reverseproxy'].append(reverseproxy_format(line))
	entries['nagios'].append(nagios_format(line))
	entries['awstats'].append(awstats_format(line))

    labFile.close()

    for key in entries:
        append_to_data(key, entries[key])
#+END_SRC
...
def append_to_data(key, data):
    ...
    newYmlData = ymlData[0:startIndex + 1] + data + ymlData[endIndex:]
...
* COMMENT Review
** [2015-08-13 Thu]
*** DONE Describe how common_vars is structured and how functions update the entries.
*** DONE Set path variable in the program itself + Environment variable
*** DONE Elaborate first line of work flow of service. also see if figure can be included.
*** DONE Throwing error when lab is already registered.
*** DONE Improve hosting definition. Hosting of a lab comprises of instantiating and
    then ...
*** DONE Make a function to read the lab.txt and then delete or add entry and throw errors.
*** DONE Hosting of a lab  = deployment + registration
*** DONE Formation of string is different for different functions rest everything is same.
*** DONE Modularise the code
** [2015-08-19 Wed]
*** DONE export path in /etc/environment
*** DONE Instead of points write literate in paragraph format.
*** DONE File structure where the python programs will sit.
*** DONE Add config server box in diagram and add apply configurationn box
*** DONE Write help for the program
*** DONE Check return codes of ssh
*** DONE Publish return codes 500, etc.
*** DONE Return error code instead of printing the error
*** DONE Write global variable definition into a config file and import it into the main file.
*** DONE Write another function for writing data to the file. Remove common code to a function.
*** DONE lab.txt location should be set in configuration file with absolute path
*** DONE Change labid to hostname
