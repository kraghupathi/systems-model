...
** Functional Requirements
   ...
   3) Push weekly backups to an off site storage node, currently
...
** Assumptions
   To have reliable secondary backup copy of this node, for better
   reliability in case of a disaster we have following assumptions

   - A remote node with sufficient storage is available
   - It accepts file transfer over SSH with =rsync=
   - It has the public key of this node as authorized key to let this
     node connect without password
   - The secondary backup node is reachable over SSH from this node
...
** System design after functional requirement 3)
   Following is the functional diagram. 

#+CAPTION: Backup setup
[[file:diagrams/rsnapshot-diagram.png][Backup setup.]]

** Source of the diagram
   Created using LibreOffice and VRT extensions
   [[./diagrams/rsnapshot-diagram.odg][Download source .odg file]]
...
#+END_EXAMPLE
   ...
   The =roles/rsnapshot_server/templates/= directory contains two
   jinja2 configuration templates - =rsnapshot.conf.j2= and
...
** Firewall Rules
   ...
   nodes are defined in the [[file:common.org::*Common Firewall Rules][common role]].

   There is a specific firewall rule to allow outgoing TCP connections
   on port 2222 to =aws-backup.vlabs.ac.in=, these connections are
   forwarded by the router of this cluster.

#+BEGIN_EXAMPLE
#Allow rsync over ssh port 22 to aws-backups.vlabs.ac.in
-A OUTPUT -d {{aws_backup}} -p tcp -m tcp --dport 2222 -j ACCEPT
#+END_EXAMPLE
...
*** =rsync= to a remote server
    For having redundant backup in case of diaster, we are having a
    backup node at IIIT-H which will be receiving backups on weekly
    basis from the AWS =rsnapshot= node. Refer the network diagram
    which represents the proposed setup.

    As the number of publicly available IP addresses are limited, we
    resort to sharing a single IP address for various services, by
    means of NAT.  As an example, a single IP address can be shared by
    both a web server and a SSH server, with the services listening on
    different ports.
    
    Here we have used the router at IIIT-H to NAT incoming connections
    on TCP port 2222 to =aws-backup.vlabs.ac.in= on TCP port 22. The
    =rsanpshot.vlabs.ac.in= server along with usual backup of systems
    servers also undertakes the task of pushing copies of backup using
    =rsync=.

    As we are backing up what was earlier backed up from other nodes,
    we need to ensure some special consideration while using =rsync=,
    which is achieved using following options.

#+BEGIN_EXAMPLE
rsync -azH --delete /path/to/source /path/to/destination
#+END_EXAMPLE

    + =-a=, which equals to =-rlptgoD=
    + =-z=, which compresses the data before transmission
    + =-H=, this tells =rynsc= to look for hard link at source and
      ensure they are hard linked at the destination, this is very
      important that it is used here.
    + =--delete=, to remove the files at destination which have been
      removed at the source

    We then add a =cron= job to automate the push of the backup files
    every week.

#+BEGIN_EXAMPLE
0 18 * * 7 rsync -azH --delete -e "ssh -p 2222" /.snapshot/* root@aws-backup.vlabs.ac.in:/destination/
#+END_EXAMPLE

The various fields of =cron= are described below.
#+BEGIN_EXAMPLE
...
*** Generate SSH keys on the =rsnapshot= server and get it to ansible server
    ...
    SSH public key is placed inside the =rsnapshost= client node's
...
*** Configure cron for backup
    Cron jobs are configured on the =rsnapshot= server to take backup of
...
*** Start iptables
    ...
    restarting the =iptables=. To restart =iptables=, handlers
    (ansible terms) are defined here.
...
rsnapshot_config_backup:
    ...
    - name: reverseproxy.vlabs.ac.in
      points:
          ...
          - [backup, "root@reverseproxy.{{prefix}}vlabs.ac.in:/var/lib/awstats", reverseproxy/]
...
rsnapshot_crontab:
     ...
     - name: rsync_job
       ...
       month: '*'
       weekday: 6
       job: "rsync -azH --delete -e 'ssh -p 2222' /.snapshot root@{{aws_backup}}:/srv"
...
** Rsnapshot server configuration script
   ...
   1) =common=
   2) =rsnapshot_server=
*** Configure rsnapshot-server
    Configure only rsnapshot-Server without installing other clients
...
#+END_SRC
*** Install clients in rsnapshot-server
    Install ossec_client,rsyslog_client,nagios_client in
    rsnapshot-server server.
    
#+BEGIN_SRC yaml :tangle install_clients_in_rsnapshot_server.yaml :eval no
---
- name: This yml script configures rsnapshot server
  hosts: rsnapshot_server
  remote_user: root
...
** Test Case-3
*** Objective
    Test/confirm whether backup node is able to push backups to
    secondary backup node.
*** Required
    1. A remote node accessible over SSH, as described in [[Assumptions]]
    2. This node configured to allow SSH connection to the secondary
       node.

*** Steps to test
    1. Log on to the =rsnapshot= node in the cluster.
    2. Run =telnet <aws-backup.vlabs.ac.in-IP> 2222= and check whether
       it connects
    3. Try transferring some file, run =rsync -aZ --progress -e 'ssh
       -p 2222' <some_file> root@aws-backup.vlabs.ac.in:.=
    4. Check whether the =rsync= job is listed in the =cron= schedule,
       open =/etc/cron.d/ansible_rsnapshot= this will list the
       scheduled job.

*** Observation
    - If step 2. passes then it means, the node can see other node
      over network
    - If step 3, shows that the file was transferred then it means
      =rsync= copy is supported on the remote node
    - If step 4, shows a scheduled job, then a copy of the backup
      directory can be expected on the secondary backup node.
