...
** Assumptions
   To have a secondary backup node for better reliability we have
...
-A OUTPUT -d {{aws-backup.vlabs.ac.in}} -p tcp -m tcp --dport 2222 -j ACCEPT
...
- name: Configure cron for rsync to aws-backup.vlabs.ac.in
  cron: cron_file=remote_backup
        minute=30
        hour=20
        day=*
        month=*
        weekday=6
        job="rsync -azH --delete -e "ssh -p 2222" /.snapshot {{remote_user}}@{{aws-backups.vlabs.ac.in}}:/srv"
