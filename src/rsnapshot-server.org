...
* Introduction
  ...
  Backup of configuration files of various nodes are saved in a
  particular node in the cluster. If a node is compromised due to any
  reason, the authenticity of the files in the node can not be
  determined. For the same reason backup of configuration files are
  not maintained locally. To setup the node again we refer backups of
  configuration files stored in a separate node.

  # Describe a bit more about why this node is important, why it
  # was decided that this should be there etc.
  # Also mention why central location and why not in the node it self.
  # Mention link to rsnapshot-client documentation in introduction as
  # well as in design section.
* Requirements
...
# Re-size the diagram to remove the extra white space around the main
# diagram. Also write some description about the diagram. How the
# network is designed etc. etc.
...
|------+-----------+-------------------------+--------------------|
# Also describe what each file is meant for. Very crisp and short
# sentence. Refer privateDNS.  also change the heading from system
# files to the same as in private-dns.
...
** Configuration Design
   ...
   - Various fields in file are separated using Tab, space does not
     separates fields
  # this description should be placed where you describe the
  # rsnapshot.conf template. You can remove this section from here and
  # add the content there in that section.
* Implementation
...
# you need not write in small pragraphs, but still it is up to you. :)
# this way of writing is also fine, but it will be better if you wrote
...
** Firewall Rules
   ...
   applied.
*** Firewall Rules
# dont make this as a separate section. Put the template in the same
# section "firewall rules" only.
...
*** Enable ssh
    Remote ssh backup over *rsync* is enabled by uncommenting
    =cmd_ssh= directive.
    # do a thorough spell check once again.
*** Backup Intervals
...
*** Local and Remote backup
    ...
    # change servers to nodes in the first sentence.
#+BEGIN_EXAMPLE
