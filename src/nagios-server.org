...
** COMMENT 
it can be "for monitoring" instead of "used to" We can add more
intorduction about nrpe-client and nagios server for system and
application level services monitoring such as cpu usage and ram usage
etc.
* Requirements
...
*** COMMENT
 Instead of "Accept " we can write "Allow" because these are
...
** System Files
*** COMMENT 
Description of System files, like purpose of this table!
...
*** COMMENT 
1. remove "<<< at the begining and end  >>>"
2. not just some specific services, we can have many services in
   system. Here you added only three services as example. we can say
   ssh,ping,http, etc,.
3. Repeated paragraphs 
4. Nagios also running on port 443
...
# templates uses different variables in various places, variables are not
# defined in various places. These varibles are defined in =vars/= folder
...
*** Update all packages
   ...
   nagios server node.
# "has to be updated" instead of "are updated"
...
# no need of "sudo: true" here.  service is restarted when associated
# task is notified to restart the service if the changes occured in
...
*** Apparatus
   1) Nagios server
   2) Nagios client
# what is nagios client here?. We may need another client machine with
# nagios-nrpe-client installed to monitor services
*** Theory
   Nagios server provides a web interface to monitor client nodes. Web
   dashboard runs as an http server on port 80.
# and also runs on https 
*** Experiment
...
# Test to send alerts when something goes wrong in local machine and
...
# after a few minutes ( 5 to 10 minutes )
*** Observation
...
*** Conclusion
   ...
   configured properly.
# you have tested this test case, so I think no need to mention "if".
** Test Case-3
*** Objective
...
# Either remove ip of client node or add nagios server ip also
# wherever we required in test cases
