...
**** Add  NAT rule and Edit =/etc/sysctl.conf= file
     ...
     #+BEGIN_EXAMPLE 
     -A POSTROUTING ! -d 10.100.0.0/22 -o eth0 -j SNAT --to-source <router_public_ip>
     #+END_EXAMPLE
...
*** Create DNS servers
    ...
    - Change public_dns_vars variable in =common_vars/vars/main.yaml=
      file.
    - We must use temporary external dns server's IP other than cluster's dns
      server just to configure these nodes. 
    - We must comment the task in common role which chages the
      =nameserver= value in =/etc/resolv.conf=. *OR* Make sure that
      the =private_dns_ips: none= and =private_dns_zone: none=
      variable value shoud be empty(none) in
      =roles/common_vars/vars/main.yaml= to configure these nodes.
      + NOTE :: If you are working under any proxy enabled network, we
                may need to check/do the following steps.

                - Cluster private dns may not connect to the
                  external network, for this to work we may have to
                  add "forwarders{10.4.12.160;10.4.12.221;};" to
                  =/etc/namd.conf= file. 
                - Check OUTPUT firewaal rule for port 8080 to connetct
                  to the proxy
                - export proxy 
                
    - Run *public-dns.yaml* and private-dns play books on config-server
      to configure public-dns.
      #+BEGIN_SRC 
    ...
    - Once the nodes are configured using playbooks, we need to test
      them whether the nodes are functioning properly or not. 
    - Once the testing is done with proper configuration,update
      varibales =private_dns_ips= and =private_dns_zone= to defualt
      values.
...
*** Enable ossec_client role on all the nodes
    - Enable ossec_client role in all the server roles. 
*** Configure machines to use private DNS
    Edit roles/common_vars/vars/main.yaml and set private_dns_ips and
    private_dns_zone values appropriately.
...
*** Configure machines to be backed up using rsnapshot server
    Add or un-comment rsnapshot_client role in all the nodes
...
*** Configure machines to be monitored by Nagios serer
    Add or un-comment nagios_client role in all the nodes
...
*** Run site.yaml on config-server
    ...
    - Comment rsnapshot_client role in all the roles. 
    - Run site.yaml file 
      #+BEGIN_SRC 
      ansible-playbook -i hosts site.yaml
      #+END_SRC
    - Un-comment rsnapshot_client role in all the roles
      #+BEGIN_SRC 
      ansible-playbook -i hosts site.yaml
      #+END_SRC
