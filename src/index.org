...
*** Setup bridged network 
    ...
    - Do the steps mentioned in the following link to install
      openvz(vzkernel) and to configure openvz
...
**** Enable router to run automatically on machine boot
     #+BEGIN_EXAMPLE
...
**** Setup container
   - Create a machine and configure the network with two IPs. Do the
     same steps as followed for router node except IPADDR and MAC
   ...
   - We need two IPs to setup ansible machine one is for public access
     ...
     #+END_EXAMPLE
     Give private IP 10.100.1.2 to ansible container.
   - Create a 'vlead' user in the machine.
     #+BEGIN_EXAMPLE
     ...
     ssh-keygen
     #+END_EXAMPLE
     ssh public key of this user has to be copied in all other
     machine.  

     If the root user has already been created then the root user's
     private and public key can be placed inside the vlead user's .ssh
     directory, so that from vlead user we can login to other machine
     roots account.
   - Enable ssh access from localhost using key-based authentication
     #+BEGIN_EXAMPLE
...
**** Version control setup
     ...
     Clone the systems-model repository inside 'vlead' user home directory. For
     this we have to add public key to repository since we do cloning over ssh
     by issuing the following command.
     #+BEGIN_EXAMPLE 
     git clone git@bitbucket.org:vlead/systems-model.git   #by vlead user
...
**** Run make and check syntax of site.yaml
    - Enter into config-server 
    - Share public key to all the nodes including itself. So that we
      can run ansible playbooks to configure nodes.
    - Change directory to cloned repository systems-model and run makefile
      #+BEGIN_SRC 
      cd systems-model/
      make
      #+END_SRC
    - Copy or move created hosts file to =build/{base1 or base4}-code/= 
    - Edit hosts file of corresponding build folder and verify that all IPs are correct
    - Check syntax
      #+BEGIN_EXAMPLE
      ansible-playbook -i bstest --list-tasks --syntax-check site.yaml
      #+END_EXAMPLE
*** Create DNS servers and Configure
    - Create following servers:
      ...
      + private DNS  
	#+BEGIN_SRC 
        (host-machine)$vzctl create CTID --ostemplate centos-6-x86_64 --hostname <server-name>.vlabs.ac.in
        (host-machine)$vzctl set CTID  --netif_add eth1,,,,br1 --save
	#+END_SRC
    ...
    - We must use temporary external dns server's IP other than
      cluster's dns server to configure these nodes using ansible
      playbooks.
    - Changes which we need to do in =roles/common_vars/vars/main.yaml= are as follows
      + =public_dns_vars= variable value with router and config-server
        public ips.
      + Comment =private_dns_ips: 10.100.1.5= , =private-dns-zone:
        base{1,4}.vlabs.ac.in= and comment out =private-dns-ips: none=
        and =private-dns-zone: none=
      + Change the value of =public-zone-address= variable (router's
        Public IP).
      + NOTE :: If you are working under any proxy enabled network, we
                may need to check/do the following steps.
        - Cluster private dns may not connect to the external network,
          for this to work we may have to add
          "forwarders{10.4.12.160;10.4.12.221;};" to
          =roles/named_server/tempalates/named.conf= template.
        
	- Check OUTPUT firewaal rule for port 8080 to connetct to the
          proxy
	- export proxy
    - Make sure that the hosts file with the ips of nodes and with
      their names according to name of the server's playbook .
    - Commenet =rsnapshot_client= and =ossec_client= roles in
    ...
    - Once the nodes are configured using playbooks, test whether the
      dns nodes are functioning properly or not.
...
*** Configure Router 
    ...
    - NOTE :: Reason to run router.yaml play book here is: We added
	      ...
	      If we set POSTROUTING firewall rule as permanent rule
              ...
              are configured. We can go to [[Run site.yaml on config-server][Run site.yaml]] to setup
              complete cluster.
...
*** Create ADS server
    - Create a container for ADS server
      #+BEGIN_SRC 
      (host-machine)$vzctl create CTID --ostemplate centos-6-x86_64 --hostname <server-name>.vlabs.ac.in
      (host-machine)$vzctl set CTID  --netif_add eth1,,,,br1 --save
      #+END_SRC
    ...
    - Install git
      ...
      #+END_SRC
    - Setup ADS server with the following steps
	...
	#+END_SRC
        It will install dependencies for openvz and for ADS server setup.
      + Clone *ovpl* 
	#+BEGIN_SRC 
	git clone https://github.com/vlead/ovpl.git
	#+END_SRC
      + change directory to ovpl and run manage_services.sh file:
	#+BEGIN_SRC 
        cd ovpl
...
*** Enable ossec_client role on all the nodes
    Enable ossec_client role in all the server roles. By default this
    is enabled in all the nodes playbooks

*** Configure machines to send logs to rsyslog server
    Add or un-comment role rsyslog_client on all servers except
    rsyslog_servers.By default this is enabled in all the nodes
    playbooks

*** Configure machines to be backed up using rsnapshot server
    By default this is disabled in all the nodes playbooks. Keep as it
    was for now.
*** Configure machines to be monitored by Nagios serer
    Add or un-comment nagios_client role in all the nodes.By default
    this is enabled in all the nodes playbooks

*** Run site.yaml on config-server
    - Make sure all necessary changes are done in
      =roles/common_vars/vars/main.yaml= appropriately.
    - Make sure that the hosts file with the ips of nodes and with
      their names according to the name of the server's playbook .
    - Check management_ips in common_vars role
    - Comment out =private_dns_ips: 10.100.1.5= , =private-dns-zone:
      base{1,4}.vlabs.ac.in= and comment =private-dns-ips: none= and
      =private-dns-zone: none= in common_vars role.
    - Add servers internal ips to =ossec_client_ips= variable in
      common_vars role.
    - Comment out =ossec_client= and =rsnapshot_client= roles in
      =private-dns.yaml= and =public-dns.yaml=
    - Comment =osse_client= role in =rsnapshot_server.yaml=
    - Run site.yaml file 
      #+BEGIN_SRC 
      ansible-playbook -i hosts site.yaml
      #+END_SRC
    - Comment out =osse_client= in =rsnapshot_server.yaml=
    - Run rsnapshot_server.yaml(instead of configuring whole cluster
      again)
      #+BEGIN_SRC 
      ansible-playbook -i hosts rsnapshot_server.yaml
      #+END_SRC
