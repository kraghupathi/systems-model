...
*** Create VM
    - NOTE :: Creating a VM is an optional. It is not compulsory.You
              can go to next step for bootstrapping process.
    ...
    - We can Create VM in two ways
      ...
      2. Using *virsh-install* command http://wiki.centos.org/HowTos/KVM
         ...
         #+END_SRC
   - After installing OS (Host machine)on created VM do the following steps 
...
**** Run make and check syntax of site.yaml
    ...
    - Run makefile. Change directory to cloned reository systems-model 
      #+BEGIN_SRC 
      $cd systems-model/
      $make
      #+END_SRC
...
*** Create DNS servers
    ...
    - We must comment the task in common role which chages the
     =nameserver= value in =/etc/resolv.conf=.
    ...
    - Once the testing is done with proper configuration, we can
      un-comment the task in common role.
...
*** Run site.yaml on config-server
    - Make sure all changes are done in
      =roles/common_vars/vars/main.yaml= appropriately. mainly
    - Un-comment the task which changes =/etc/resolv.conf= in
      =roles/common/tasks/main.yaml= to use private dns ip as
      nameserver.
