...
*** Setup base machine with OpenVZ etc. for ADS
	...
	#+END_SRC
        It will install dependencies for openvz and for ADS server
        setup.
...
*** Create ADS server
    - Create a container for ADS server
      #+BEGIN_SRC 
      vzctl create 1009 --ostemplate centos-6-x86_64 --hostname ads.[{cluster}.].vlabs.ac.in
    ...
    - Install git
      ...
      yum -y install git
      #+END_SRC
    - Generate root users ssh-keys.  Setup trust based ssh from OVPL
    ...
    - Edit adapters/base_config.py as follows:
      ...
      + HOST_NAME = "bstest.vlabs.ac.in"
        + Domain name of lab-id
    - Again clone setup-ovpl-centos using:
      #+BEGIN_EXAMPLE
      git clone https://github.com/vlead/setup-ovpl-centos.git
      cd setup-ovpl-centos/scripts
      vim centos_prepare_ovpl.sh  #Comment OpenVZ installation lines 52,58
      ./centos_prepare_ovpl.sh
      #+END_EXAMPLE
    - Run services using
      #+BEGIN_EXAMPLE
