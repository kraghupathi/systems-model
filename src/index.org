...
*** Ansible machine creation
    ...
    #+END_EXAMPLE
    to install ansible. Create a 'vlead' user in the machine.
    #+BEGIN_EXAMPLE
    adduser vlead 
    #+END_EXAMPLE
    ssh public key of this user has to be copied in all other machine.
...
*** Create Config-server node
   ...
   - Setup rpmfusion-free, rpmfusion-nonfree, rpmforge and
     ...
     install ansible
     #+BEGIN_EXAMPLE 
     yum -y install epel-release  ansible -y
     #+END_EXAMPLE
...
**** Run make and check syntax of site.yaml
    ...
    - Edit or create hosts file to point to correct servers/nodes.
    - Change directory to cloned reository systems-model and run makefile
      #+BEGIN_SRC 
      $cd systems-model/
      $make
      #+END_SRC
...
*** Create DNS servers
    ...
    - We must use temporary external dns server's IP other than cluster's dns
      server just to configure these nodes. 
    - Changes which we need to do in =roles/common_vars/vars/main.yaml= are as follows
      ...
      + Change the value of =public-zone-address= variable.
      + NOTE :: If you are working under any proxy enabled network, we
        ...
        - Cluster private dns may not connect to the external network,
          ...
          "forwarders{10.4.12.160;10.4.12.221;};" to =/etc/namd.conf=
          file.
    ...
    - Run *public_dns.yaml* and *private_dns.yaml* play books on
      ...
      ansible-playbook -i hosts private-dns.yaml
      #+END_SRC
    - Once the nodes are configured using playbooks, we need to test
      them whether the dns nodes are functioning properly or not.
...
*** Create ADS server
    ...
    - Enable ssh access from ansible server
    - Install git
      #+BEGIN_SRC 
...
*** Run site.yaml on config-server
    - Make sure all changes are done in
    ...
    - Add servers internal ips to =ossec_client_ips= variable in
      common_vars role.
    - Commet ossec_client role in all the servers playbooks since
      model not yet done.
    - Run site.yaml file 
      #+BEGIN_SRC 
      ansible-playbook -i hosts site.yaml
      #+END_SRC
    - Un-comment rsnapshot_client role in all the roles and run
      #+BEGIN_SRC 
      ansible-playbook -i hosts site.yaml
      #+END_SRC
*** Another way to run site.yaml 
    If we want to run directly site.yaml from config-server after
    creating all containers then do the following steps:
    - Comment =private_dns_ips: 10.100.1.5= , =private-dns-zone:
      base{1,4}.vlabs.ac.in= and comment out =private-dns-ips: none= and
      =private-dns-zone: none= in common_vars role.
    - Update all other variable values which are required in
      common_vars role
    - comment ossec_client role in all the nodes
    - Run site.yaml
    - Comment out =private_dns_ips: 10.100.1.5= , =private-dns-zone:
      base{1,4}.vlabs.ac.in= and comment =private-dns-ips: none= and
      =private-dns-zone: none= in common_vars role.
    - comment out rsnapshot_client role in all the servers playbooks
    - run site.yaml
