...
- Do the steps mentioned at this link
...
** Router 
    We need two IPS to setup router machine one is for public access
    over ssh and another is for private network ip. This private ip is
    for gateway to all the nodes and lab containers. 
*** Create router node 
    - Assume you have centos template ready to create containers on
      host machine
...
The router needs to be connected to two bridges br0 and br1 in order
to get internet connection and to setup private network via this
router. Do the following commands
#+begin_src 
(host-machine)$vzctl set 1001 --netif_add eth0,,,,br0 --save
(host-machine)$vzctl set 1001 --netif_add eth1,,,,br1 --save
#+end_src 
  It means container will be created with two interfaces eth0
  and eth1 and eth0 of the router is connected to br0 of host machine
...
Create *ifcfg-eth0* file in =/etc/sysconfig/network-scripts/= and add
the following fields. In place of HWADDR add MAC address of eth0
interface.
#+begin_src 
DEVICE=eth0
...
#+end_src
- Check the link for above things done router
https://openvz.org/Using_NAT_for_container_with_private_IPs

** Config-server node
   - We need two IPs to setup ansible machine one is for public
     access over ssh and another is for private network ip. This
     private ip is for configuring other nodes as well as itself and
     internal connections.

   - Create a machine and configure network with two ips. Do the same
     steps as router node except IPADDR and MAC addresses of
     interfaces. 
...
#+BEGIN_EXAMPLE
     yum -y install git
#+END_EXAMPLE
    In this node, git is to pull the updates from master branch
    systems model. From this node we do not push the updated
    files/folders to systems-model repo. For more details look at
    [[./config-server.org][Ansible machine]] model.
