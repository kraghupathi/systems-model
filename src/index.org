...
**** Setup container
   - Create a machine and configure the network with two IPs. Do the
     same steps as followed for router node, set IPADDR and MAC
     addresses of the interfaces appropriately.
     #+BEGIN_EXAMPLE
...
**** Version control setup
     ...
     Create .ssh/config file with following content
#+begin_example
...
#+end_example
     ...
     #+BEGIN_EXAMPLE 
     git clone git@bitbucket.org:vlead/systems-model.git   #from vlead user
