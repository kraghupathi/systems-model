...
*** Host labs using ads service
    - Deploy a few labs using ads service to test the whole
      ...
      required fields.
    - After deploying the lab update the entries in common_vars to
      access the lab using fqdn. Then run ansible-playbooks of three
      roles as follows:
#+begin_example
ansible-playbook -i hosts reverseproxy_server.yaml
ansible-playbook -i hosts pubcli-dns.yaml
ansible-playbook -i hosts private-dns.yaml
#+end_example
