...
2) Allow incoming ssh connections on TCP port 22 from Ansible, Nagios and
   management servers.
...
- Firwall rules applied to rsyslog server are mentioned here. Description of
  firewall rule which are common across all the systems are defined in common
  role. Rule [[Rule for FORWARD chain][1]] and [[Rule for incoming connection for rsyslog messages][2]] are specific to only rsyslog server and are described here.
...
** Test Case-1
*** Objective: 
- Test firewall rules to accept incoming connections on port UDP 514
*** Apparatus:
1) Rsyslog server
2) Rsyslog client
...
*** Experiment
- Login to rsyslog server from one terminal and go to
  =/var/log/<clienthostname>= directory. Do "tail -f su.log". Now from another
  terminal login to rsyslog client machine. Execute "sudo su -" command. When we
  login to the client machine a session gets created for the user and a system
  log is generated. The log message will be forwareded to rsyslog server and
  saved in a file. As a result messages should appear in the first terminal
  screen.
...
** Test Case-2
*** Objective:
- Test ssh log are getting logged in rsyslog server
...
*** Theory
- System generates log message everytime an event occurs in the system. When we
  login inside the system using ssh, machine generates "ssh log" message. This
  message is then forwarded to rsyslog server.

*** Experiment
- Login to rsyslog server from one terminal and go to
  =/var/log/<client-hostname>= directory. Monitor =sshd.log= file using 
  "tail -f sshd.log" command. Now from another terminal login to rsyslog client
  machine via ssh. The ssh log message is generated which is then forwarded to
  rsyslog server and saved in a file. As a result messages should appear in the
...
- Apache saves a log message everytime an an HTTP request is made to apache
  server.  By default apache saves log messages in
  =/var/log/httpd/access_log=. These logs can be forwarded to a central rsyslog
  server.  Configure apache to pipes the log messages to external program by
  changing the value of "CustomLog" directive as follows:
   #+BEGIN_EXAMPLE
