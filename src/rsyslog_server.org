...
3) Save log messages forwarded by various client systems in a separate file for
   each client.
...
which configures the machine. These scripts are organized as follows:
#+BEGIN_EXAMPLE
|-code
|   |-- rsyslog_server.yml
|   |-- roles
...
role. Rules [[Rule for incoming connection for rsyslog messages][1]] and [[Rule for FORWARD chain][2]] and are specific to only rsyslog server and are described
here.
...
- name: Updating Hostname
  lineinfile: dest=/etc/sysconfig/network regexp=^HOSTNAME= 'line=HOSTNAME="rsyslog.virtual-labs.ac.in"'
#+END_SRC
...
Set rsyslog service to auto start when the system boots up.
#+BEGIN_SRC yml :tangle roles/rsyslog_server/tasks/main.yml
...
Any changes in iptables configuration file to update the firewall is enforced by
...
machine. Execute "sudo su -" command. Once login to the client machine a session
...
sshd.log= command. Now from another terminal login to rsyslog client machine via
