...
* Introduction
  This document contains automation scripts for ubuntu-os-Hardening.
...
*** Configure mail transfer agent for local-only mode
   [[https://gitlab.com/vlead-systems/vlead-security-audit/issues/564][Associated issue on GitLab]]   
   - Impact :: Network-based attacks

   - Recommendations :: Edit =/etc/postfix/main.cf= and add the
        following line to the RECEIVING MAIL section.

     If the line already exists, change it to look like the line
        below:
   ...
   - Implementation ::
#+BEGIN_SRC yml :tangle roles/ubuntu_harden/tasks/main.yml
- name: Configuring mail transfer agent for local-only mode
  lineinfile:
...
** Create application service hosts
   Replace following services with ip address, Ex:
   <analytics-db-ip-address> = 10.100.2.3
#+BEGIN_SRC text :tangle service-hosts
[services]
<analytics-db-ip-address>
<vlabs-analytics-ip-address>
<feedback-ip-address>
<outreach-ip-address>
#+END_SRC
