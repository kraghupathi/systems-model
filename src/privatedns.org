...
#+END_EXAMPLE
  ...
  Here =private_dns.yml= file configures the private DNS with the
  private_dns and common roles as described in the [[Creating%20the%20Private%20DNS][Creating the
  ...
  The =roles/named-server/templates/= contains the various
  configurations file templates with the variables used to configure
  the service in the required manner.
