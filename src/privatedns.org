...
avoid any break in the service of the DNS.  In our architecture we
have removed this dependency.  We use the configuration management
server to configure the DNS server. In case this gets compromised we
can setup another DNS with the same configuration and data as the
previous one hence removing the dependency on the master/salve setup.
