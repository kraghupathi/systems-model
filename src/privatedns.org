...
domain names (ex:gnu.org). 

Generally the DNS is configured with the master/slave style of
configuration. This is done to have a fallback machine as the slave in
case master DNS server is compromised. The slave machine is used to
serve as the DNS in the absence of the master. This usually helps to
avoid a break in the service of the DNS.  In our architecture we have
removed this dependency.  We use the configuration management server
to configure the DNS server.  In case this gets compromised we can
setup another DNS with the same configuration and data as the previous
one hence removing the dependency on the master/salve setup.

* Requirements
The functional and security requirements of the Private DNS are
mentioned in the following sections. These are specific to the Private
DNS. The requirements which are common to all the servers are
mentioned in the [[common%20role][common role]].
** Functional Requirements
   The Private DNS should resolve our authoritative domain names for
   all internal clients on UDP port 53. It should also resolve all
   domain names for internal clients (Peer DNS). The scripts used to
   configure it should be such that the zone entries should be
   configurable (from a particular file on configuration management
   server).

** Security
   All common security requirements as described in common role are
   applicable to the private DNS. Along with them port 53 (UDP) should
   be allowed *only* from any machine in the cluster.
...
#+END_EXAMPLE
  ...
  notifies them.  Handlers are described in detail in the [[Handlers]]
  section.  The handlers are called only when the tasks described in
  the following 5 sections (4.2,4.3,4.4,4.5,4.6) are called as a part
  of the named_server role.  The code from these 5 sections
  concatenate into one place =roles/named_server/tasks/main.yaml=.
  The =roles/named-server/templates/= folder contains the various
  configuration file templates with the variables used to configure
  the service in the required manner. Three templates are used
  =named_iptables=, =zone.forward=, =named.conf=. These are described
...
** Configuring the Bind service
   ...
   domain. The task uses the template
   =/roles/named_server/templates/named.conf= to configure the bind
   service. Once this is configured the task notifies the
   handler. This is one by the =notify= module of ansible.
...
- name: Copy zone forward files for all zones to /var/named
  ...
  notify:
    - restart bind
#+END_SRC
Once the zone file is configured the task notifies the handlers using
the =notify= module of ansible.  The zone file is configured using the
template =/roles/named_server/templates/zone.forward=.  The zone file
...
** Building the  firewall rules
   ...
   listed below. This template contains the common firewall rules
   also. These are described more in detail in the [[common.org][common role]].  The
   rules which are specific to the private DNS are also mentioned here
   in this template.
...
#
#+END_SRC
 Generally the other servers are allowed only to send DNS
 queries. These queries are answered by the private DNS. The private
 DNS should be able to allow the incoming DNS queries on port 53. The
 =INPUT= table has a rule which specifies this access.
