...
*** Configure Public DNS server
    Configure public-dns without installing other clients
...
*** Install clients in  Public DNS server
    Install ossec_client,rsyslog_client,nagios_client in Public-dns-server
    server.
    
#+BEGIN_SRC YAML :tangle install_clients_in_public_dns.yaml 
---
- name: This file configures public dns server
  hosts: public_dns
  remote_user: root
