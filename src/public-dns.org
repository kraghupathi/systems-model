...
* Introduction
  ...
  of the public *Domain Name Server (DNS)* .  This server is used to
  ...
  domain name "virtual-labs.ac.in" ans "vlabs.ac.in". The Public IP of
...
# In place of word 'server', 'node' could be used.
# Sentence 'This DNS will be ... ' could be reframed as 'This DNS is
# the authoritative ..'.
# Spelling of 'and' is wrong in "virtual-labs.ac.in" ans
# "vlabs.ac.in".
# Again sentence "The Public IP of this machine needs .." could be
# reframed as "The Public IP of this machine is officially .."

# In this section, how outside users connect to public DNS could be
# described, also about its role in the whole infrastructure which is
# to provide name resolution for public domains.
...
** Cluster specific design constraints
   ...
   IIIT network. Hence these clusters are setup as sub-zones in the
   private DNS.  The query first reaches the private DNS in IIIT and
   then is forwarded to the public DNS of the cluster to respond to
   the query.

# Sentence /Eventually this DNS will also be registered on AWS/ is not
# clear. DNS will be registered on AWS or a public DNS will also be
# setup in AWS.
# Sentence 'Hence these clusters are ..' could be more elaborated
# something like 'sub zones in the private DNS are created to resolves
# domain names of nodes setup in base1 and base4'

** Important configuration and data files and folders 
   The following table gives the information about the configuration
   files which may need to be modified to setup a DNS server. The
   =bind= package is used to configure the DNS server.
   |------+----------+-------------------------------+----------------------------------|
   | S.no | Service  | File                          | Purpose                          |
   |------+----------+-------------------------------+----------------------------------|
...
# These configurations files are modified, then why are we writing
...
# Sentence 'in case a task notifies them' is repeated again in the
# later sentence 'the tasks described in the [[Tasks][Tasks]] section notify
# them', probably this is redundant information.
...
#+END_SRC
   ...
   port 53 from anywhere. The =INPUT= table has a rule which specifies
   this rule to allow DNS queries from everywhere.
  
# Line numbers at which specific firewall rules are defined could be
# mentioned. To get line numbers in source block '-n' flag could be
# used.

** Tasks
*** Installing the bind service
    To configure the node as a DNS server the *bind* and *bind-utils*
    packages have to be installed.  We are currently using
    /bind-9.8.2-0.30.rc1.el6_6.2.x86_64/ package.  This is done by the
    following code:
...
# Sentence '.. FQDN should be sent to' could be written as 'IP address
# to which the request for that FQDN resolve to'. Word 'sent' does not
# seems to be appropriate here.
# In last sentence '... looks like the following' word 'looks like'
# does not seems to be appropriate, it could be removed.
...
# Section heading could be changed 'Disabling IPv6' instead of 'Some
# more configuration'.
# Description why ipv6 is disabled could be provided.
...
*** Apply the firewall rules
    ...
    configured by placing need to be applied. This is done from the
    following script.

# There is some ambiguity in 'need to be configured by placing need to
# be applied' sentence, it could be corrected.
...
# In the procedure section only one input to 'nslookup' command is
# specified i.e. '<FQDN>'. Here in the experiment section, second
# parameter as ip address is also supplied. So it could be better if
# description of second parameter is also provided either in procedure
# or experiment section.
...
# What is 'first command' ? Instead 'nslookup' command could be used.
...
# What is 'second command' ? Instead 'dig' command could be used.
...
# In the observation what 'dig' and 'nslookup' command does are more
# described. Only sentences from 'The given FQDN was
# =ossec-server.base1.virtual-labs.ac.in= ....' seems to be sufficient.
...
# In which machine these commands are executed is not mentioned. It
# could be better to mention that.
...
# Sentence 'Dig also shows that the same result' could be corrected.
