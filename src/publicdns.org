...
servers in the cluster. The name "Public" is a term coined by us to
distinguish the primary functionality of the DNS. This DNS will be the
authoritative name server for our domain name "virtual-labs.ac.in".
The Public IP of this machine needs to be officially registered with
ERNET to make this machine an authoritative name server for our
domain.

* Requirements
The functional and security requirements of the Public DNS are
mentioned in the following sections. These are specific the Public
...
Public DNS with the other machines in the cluster. This diagram
...
*** Objective
    ...
    the hostnames of machines which have been configured in the Public
    DNS zone file.
*** Apparatus
...
** Test case ID: TC02
*** Objective
    The objective is to test that the Public DNS should not work as a Peer DNS.
*** Apparatus
    - The Private DNS container/machine
    - Another machine in the same network.
*** Theory
...
** Test case ID: TC03
*** Objective
    The objective is to test that the Public DNS should not resolve the internal IPs of machines.
*** Apparatus
    - The Private DNS container/machine
    - Another machine in the same network.
*** Theory
...
dig google.com @10.100.1.6

; <<>> DiG 9.8.2rc1-RedHat-9.8.2-0.30.rc1.el6_6.2 <<>> google.com @10.100.1.6
;; global options: +cmd
;; Got answer:
;; ->>HEADER<<- opcode: QUERY, status: REFUSED, id: 10262
...
;; Query time: 1 msec
;; SERVER: 10.100.1.6#53(10.100.1.6)
;; WHEN: Tue Apr  7 02:54:38 2015
;; MSG SIZE  rcvd: 28
