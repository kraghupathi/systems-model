...
* Introduction
  ...
  variables used by jinja2 template in various other ansible
  roles. Examples and verifications are given wherever possible.
  # used by jinja2 "templates".  Also explain the importance/purpose
  # of this role and why we took the decision to have a role called
  # common-vars. As this is not generally the way we use roles in
  # ansible, we need to specify this. Also mention generally we use
  # the vars facility provided by ansible but this does not meet our
  # requirement, hence we took this decision etc etc..
* Requirements
  The functional requirements of the Commonvars role are mentioned in
  the following sections.
  # Commonvars -> common-vars / common vars.
** Functional Requirements
   1) Define variables used by various other ansible roles
   # remove the numner. As there is only one point. Also add a little
   # more description for this point.
* Implementation
...
** Generate yml files of common_vars role
   ...
   # incorporate the prefix in the code also for all the three
   # clusters. Check with aws-implementation code also. If it is not
   # added there no problem we need to add it here.
