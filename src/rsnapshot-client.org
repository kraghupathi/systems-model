...
* Introduction
  ...
  # Same as in server! please describe what central node is or mention
  # "as described in the server documentation".  Also give a
  # description as to what the client is required or why this is
  # required and what dos it do. In little easy to understand words.
* Requirements
...
** Security Requirements
   ...
   # make it as a single statement. Since there is only one point, do
   # not mention the numbers.
* Design
...
[[./diagrams/rsnapshot-client-design-diagram.png]]
   # give the caption and the figure name for the image also give
   # description for the diagram explaining what it depicts.
...
|------+----------+----------------------------+--------------------|
    # same as in server. Rename this to "Important configuration and
    # data files and folders". also add a crisp description for each
    # file.
** Configuration Design
   Rsnapshot server rsync's the files to be backed up from all the
   nodes in the cluster over ssh.
   # remove this section and add the contents in the corresponding
...
#+END_EXAMPLE
   ...
   # Same as in server.. Make them as a single paragraph. 
** Firewall Rules
...
# If there are no rules specific to rsnapshot client, then please
# remove the first line. The second sentence is apt enough.
** Tasks
...
** Variables Definition
   Rsnapshost server's ssh-public keys are to be placed in the
