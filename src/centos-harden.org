...
*** Password creation requirements
    [[https://gitlab.com/vlead-systems/vlead-security-audit/issues/181][Check the issue on GitLab]]
   - Impact :: Strong password protect systems from being hacked
               through brute force methods.

    - Recommendations ::
			Edit the =/etc/pam.d/password-auth= and
                        =/etc/pam.d/system-auth= files to include the
                        appropriate options for pam_cracklib.so and to
                        conform to site policy:
...
#+BEGIN_SRC yml :tangle  roles/centos_harden/tasks/main.yml
- name: Creation of password requirements
  lineinfile: 
   name: "{{ item }}"
